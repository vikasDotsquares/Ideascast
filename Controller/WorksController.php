...
class WorksController extends AppController {
	...
	public function index($level = null) {
		...
		$current_user_id = $user_id = $this->Session->read('Auth.User.id');
		$ownersProjects = $this->ownerProjects($current_user_id, 0);
		...
		if (isset($ownersProjects) && isset($sharerProjects) && !empty($ownersProjects) && !empty($sharerProjects)) {
			...
			$between = ''; 
					...
					$select = "SELECT id,title FROM projects as Project WHERE id IN (" . $arrayTostr . ") " . $between.$orderby;
					//echo $select
		...
		$this->set('projectsids', $projectIds);
		$this->set('elementidsstring', $elementidsstring);		
		...
		if ($this->request->isAjax()) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				if (isset($this->request->data['requestTyep']) && $this->request->data['requestTyep'] == 'projects') {
					...
					$this->set('projectsids', $projectIds);
					$this->set('elementidsstring', $elementidsstring);
					$this->set('totalcntprojects', count($projectIds));
					$this->set('user_id', $user_id);
					$this->set('OwnerSharerUsers', $OwnerSharerUsers); 

					$this->render('/Works/partials/workcenterlist');
	...
	public function project_users() {
		...
		if ($this->request->isAjax()) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				$this->render('/Works/project_users');
	...
	public function taskcalendar() {
		...
		if (isset($this->request->data['id']) && !empty($this->request->data['id'])) {
			...
			//$this->render('/Works/partials/taskcalendar');
	...
	public function projectelements() {
		...
		if ($this->request->isAjax()) {
			...
			$this->render('/Works/partials/projectelements');
	...
	public function projectallelements() {
		...
		if ($this->request->isAjax()) {
			...
			$this->set('response', $response);
			$this->render('/Works/partials/projectallelements');
	...
	public function userprojectelements() {
		...
		if ($this->request->isAjax()) {
			...
			$this->set('response', $response);
			$this->render('/Works/partials/userprojectelements');
	...
	public function not_available_dates() {
		...
		if ($this->request->isAjax()) {
			...
			// $this->render('/Works/partials/not_available_dates');
	...
	public function getUserProjectPermissionDates($user_id = null, $start_Date = null, $end_Date = null) {
		...
		$ownersProjects = $Works->ownerProjects($current_user_id, 1);
		$sharerProjects = $Works->ownerProjects($user_id); 
...
/************ PROJECT SCENARIO ***************/
	public function project_scenario() {
		...
		$this->set($viewData);
		$this->render(DS . 'Works' . DS . 'partials' . DS . 'project_scenario');
	}
	...
	/*************************************************************************************
	************	New function for Workcenter							******************	  
	**************************************************************************************/
	
	public function project_elements(){
		$this->layout = false;
		
		$TaskCenter = $this->objView->loadHelper('TaskCenter');
		...
		if ($this->request->isAjax()) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				$p_permission = $Common->project_permission_details($project_id,$user_id);
				$user_project = $Common->userprojectwc($project_id,$user_id);
				$grp_id = $Group->GroupIDbyUserID($project_id,$user_id);
				...
				if((isset($user_project) && !empty($user_project)) ||  (isset($p_permission['ProjectPermission']['project_level']) && $p_permission['ProjectPermission']['project_level']==1) || (isset($project_level) && $project_level==1) ) {
					...
					$wsList = get_project_workspace($project_id,1);
					$wsp_ids = array();
					if( isset($wsList) && !empty($wsList) ){
						$wsp_ids = array_keys($wsList);
					}
				...
				$viewData = ['projectIds'=>$project_id,'wsp_elements'=>$wsp_elements,'elements_by_date'=>$elements_by_date,'projectElementCount'=>$projectElementCount,'user_id'=>$user_id,'current_user_id'=>$this->Session->read('Auth.User.id')];
				
				$view1 = new View($this, false);
				$view1->viewPath = 'Works/partials';
				$view1->set($viewData);
				$html = $view1->render('project_elements');
				
				$response = [
					'success' => true,
					'content' => $html,
					'eleids' => $elementidsstring
				];	
				echo json_encode($response);
				exit();				
	...
	public function project_calendar(){
		...
		if ($this->request->isAjax()) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				$this->render('/Works/partials/project_calendar');		
