...
class ProjectHubsController extends AppController {

	public $name = 'ProjectHubs';
	...
	public function index($level = null) {
		...
		$this->layout = 'inner';
		$this->set('title_for_layout', __('Project Hub', true));
		$this->set('page_heading', __('Project Hub', true));
		$this->set('page_subheading', __('View and navigate to your projects', true));
		...
		$myreceivedprojectlist = groupprojects($this->user_id, $level);
		// Find All current user's group projects
		$mygroupprojectlist = receivedprojects($this->user_id, $level);
		...
		if ((isset($conditions) && !empty($conditions)) || !empty($projects1)) {
			...
			$MyProjects = $this->UserProject->find('all', array('conditions' => $conditions, 'recursive' => 1, 'fields' => ['Project.id', 'Project.title'], 'ORDER'=>'modified DESC'));
		...
		$crumb = [
			'Project' => [
				'data' => [
					'url' => '/project_hubs/index/',
					'class' => 'tipText',
					'title' => 'Project Hub',
					'data-original-title' => 'Project Hub',
				],
			],
		];
	...
	public function workcenterlist() {
		...
		if ($this->request->isAjax()) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				$this->request->data['requestTyep'] = "projects";
				if (isset($this->request->data['requestTyep']) && $this->request->data['requestTyep'] == 'projects' && !empty($this->request->data['projects']) ) { 

					$projectIds = array();
					if (isset($this->request->data['projects']) && !empty($this->request->data['projects'])) {
						
						$projectIds = $this->request->data['projects']; 
					}
					
					$this->set('projects', $projectIds);
					$this->render('/ProjectHubs/partials/workcenterlist');
	...
	public function project_users() {
		...
		if ($this->request->isAjax()) {
			...
			if ($this->request->is('post') || $this->request->is('put')) {
				...
				$this->render('/project_hubs/project_users');
		...
		if (isset($this->request->data['id']) && !empty($this->request->data['id'])) {
			...
			if (isset($elements) && !empty($elements)) {
				foreach ($elements as $key => $valE) {
					...
					$calenderElementsArr[$valE['Element']['id']]['website_url'] = strip_tags(SITEURL . "project_hubs/projectelements/");
			...
			//$this->render('/ProjectHubs/partials/taskcalendar');
	...
	public function projectelements() {
		...
		if ($this->request->isAjax()) {
			...
			$this->render('/project_hubs/partials/projectelements');
	...
	public function userprojectelements() {
		...
		if ($this->request->isAjax()) {
			...
			$this->set('response', $response);
			$this->render('/project_hubs/partials/userprojectelements');
	...
	public function not_available_dates() {
		...
		if ($this->request->isAjax()) {
			...
			$view->viewPath = 'project_hubs/partials';
			...
			// $this->render('/project_hubs/partials/not_available_dates');
...
/************ PROJECT SCENARIO ***************/
	public function project_scenario() {
		...
		$this->set($viewData);
		$this->render(DS . 'project_hubs' . DS . 'partials' . DS . 'project_scenario');
	}
