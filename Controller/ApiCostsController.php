 ...
 */
class ApiCostsController extends AppController {
	...
	public function beforeFilter() {
		...
		parent::beforeFilter();
		$this->Auth->allow('get_project_cost_details');
		$view = new View();
		$this->objView = $view;
	...
	public function get_project_cost_details()
	{
		$response = array();
		$data = array();
		...
		else
		{
			...
			else
			{
				...
				$workspaces = $this->Workspace->find('all',array('conditions'=>array('Workspace.id'=>$workSpIds)));
				$project['Workspaces'] = array();
				...
				$elemIds = array();
				foreach($project['Workspaces'] as $wkey => $work )
				{
					$work_id = $work['id'];
					$areaElementdata = $this->Area->find('all', array('conditions'=>array('Area.workspace_id'=>$work_id)) );

					foreach($areaElementdata[$wkey]['Elements'] as $key => $eleData)
					{
						$elemIds[] = $eleData['id'];
						$areaElementdata[$wkey]['Elements'][$key]['estimated_cost'] = number_format($this->objView->loadHelper('ViewModel')->wsp_element_cost($eleData['id'],1), 2, '.', '');
						$areaElementdata[$wkey]['Elements'][$key]['spend_cost'] = number_format($this->objView->loadHelper('ViewModel')->wsp_element_cost($eleData['id'],2), 2, '.', '');
					}
					$estimated_cost =  0 ;
					$spend_cost= 0 ;
					foreach($elemIds as $ele)
					{
						$estimated_cost +=  $this->objView->loadHelper('ViewModel')->wsp_element_cost($ele,1);
						$spend_cost +=  $this->objView->loadHelper('ViewModel')->wsp_element_cost($ele,2);
					}
					$project['Workspaces'][$wkey]['estimated_cost'] = number_format($estimated_cost, 2, '.', '');
					$project['Workspaces'][$wkey]['spend_cost'] = number_format($spend_cost, 2, '.', '');
					$project['Workspaces'][$wkey]['Elements'] = array();
					$project['Workspaces'][$wkey]['Elements'] = $areaElementdata[$wkey]['Elements'];
				}
				...
				$keysArray = array(
						'Project' => array('id','title','objective','description','rag_current_status','created','modified','start_date','end_date','current_status','color_code','status','budget'),
