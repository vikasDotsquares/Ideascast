...
#popup_model_box{ z-index : +99999999999;}
...
<script type="text/javascript" src="<?php echo SITEURL;?>js/projects/plugins/tooltip/src/jquery.poshytip.js"></script>
...
<script type="text/javascript" >
   ...
   // assignment filter
   $('body').delegate(".assignmentfilter", 'click', function (e) {

	 var element_ids = $(this).data('element');
	 var project_id = $(this).data('project');
	 console.log(element_ids)
	 console.log(project_id)

		$(".getelements").trigger("click",[{priority:0,user_id:0,group_id:0,sharing_type:0,assign_type:'assignfilter'}]);
		...
		$('#modal_medium').on('hidden.bs.modal', function(){
			...
			$('.wrapper').nextAll('.gantt_task_line.panel').remove();
		})

		$('#modal_large').on('hidden.bs.modal', function(){
				$(this).removeData()
				$(".gantt_tree_content").tooltip('destroy');
				setTimeout(function(){
    ...
    $('#modal_pop').on('show.bs.modal', function(){
		$(this).removeData('bs.modal');
		$(this).find('.modal-content').html('');
	});

	$('#modal_priority').on('show.bs.modal', function(){

	});

	/* $('#modal_priority').on('hidden.bs.modal', function(){
		$(this).removeData('bs.modal')
		$(this).find('.modal-content').html('')
	}) */

	$('#modal_medium').on('show.bs.modal', function(){
		$(this).removeData('bs.modal')
		$(this).find('.modal-content').html('')
	...
	setTimeout(function(){
		...
		setTimeout(function(){
          $('.gantt_tree_content').tooltip({
                html: true,
                container: 'body',
				template: '<div class="tooltip tooltip-custom"><div class="title"></div><div class="tooltip-arrow"></div><div class="tooltip-inner adjusted-align"></div></div>'
		...
		$('#modal_medium').on('hide.bs.modal', function(){
			console.log('closed');
			setTimeout(function(){
			  $('.gantt_tree_content').tooltip({
					html: true,
		 ...
		 $('.tipText').tooltip({  container: "body"});

		$('body').delegate(".gantt_tree_icon", 'click', function (e) {
		...
		function getVisible(dd) {
		 ...
		 var possign = 'bottom';
		$('body').delegate(".gantt_task_line", 'mouseover', function (e) {
		...
		//$('.gantt_task_line').popover('show');


		})


       $('body').delegate(".sidebar-toggle", 'click', function (e) {
			e.preventDefault();
       ...
       // Change select box my project and received project
/*        $(".project_type").click(function () {
           ...
           if (id == 'my_received_project') {
               $("#event_gantt").attr("action", "<?php echo SITEURL; ?>users/event_gantt/r_project:" + current_val);
           }
           if (id == 'my_project') {
               $("#event_gantt").attr("action", "<?php echo SITEURL; ?>users/event_gantt/m_project:" + current_val);
           }
           if (id == 'group_received_project') {
               $("#event_gantt").attr("action", "<?php echo SITEURL; ?>users/event_gantt/g_project:" + current_val);
           }
           ...
           if (current_val != ''  && current_val != null) {
               $("#event_gantt").submit();
                ...
                $("#gantt_block").html('<div class="text-center"> No Recored Found!</div>');
            }

       }) */

       // change select box to get workspaces
       $(".getWrkSpaces").change(function () {
           ...
           var select_box_type = $(this).attr("id");
		    ...
		    $("#event_gantt").attr("action", "<?php echo SITEURL; ?>users/event_gantt/" + current_val);
			if(current_val !=''){
               $("#event_gantt").submit();
			}
       });
	   ...
	   $('body').delegate(".ganttprioritytsk", 'click', function (e) {
		  $(".getelements").trigger("click",[{priority:1}]);
	   });

	   $('body').delegate(".myaccountPic", 'click', function (e) {

		  $(".getelements").trigger("click",[{user_id:$(this).data('user'),group_id:$(this).data('group'),sharing_type:$(this).data('permission')}]);
	   });


	 //  console.log("test");
	   $('.gantt_tree_icon').attr('cursor','pointer');

	//  $(".gantt_tree_icon").click(function (event, extraData = {priority:0,user_id:0,group_id:0,sharing_type:0,assign_type:'none'}) {



       $(".getelements").click(function (event, extraData = {priority:0,user_id:0,group_id:0,sharing_type:0,assign_type:'none'}) {
		  ...
		  /*  if (workspaceid == '') {
               var srcvalue = "<?php echo SITEURL; ?>users/get_workspaces_by_project/<?php echo $project_id ?>/mode:<?php echo $mode;?>";
           } else {
               var srcvalue = "<?php echo SITEURL; ?>users/get_element_by_workspaces/<?php echo $project_id ?>/workspace_id:" + workspaceid+'/mode:<?php echo $mode;?>';
           } */
           ...
           $.ajax({
               ...
               complete: function () {
                  // $(".ajax_overlay_preloader").fadeOut();
				   //	console.log("comp");
				   if(extraData.priority == 1){
							//$('.gantt_row').hide()
							//console.log("comp");
							//$('.gantt_row').find('.gantt_close').show()
				   }
				 //  $('#gantt_block').show();
				 //  alert(0)
               },
               data: {"project_id": projectid, "project_type": project_type, "workspace": workspaceid,"status":status,"criticalStatus":elementCritical,"user_id":user_id,"group_id":group_id,"sharing_type":sharing_type,"assign_type":assign_type}
           });
