...
$workspacSpendCost = $projectSpendTotal = 0;
...
if(isset($wsp_tasks_data) && !empty($wsp_tasks_data)){
	foreach ($wsp_tasks_data as $key => $value) {
		...
		$projectSpendTotal += $spend_total;

		$all_elements = explode(',', $all_tasks);

		if( $project_level == 1 ) {
			$wsp_html = "<a class='perm' href='".$costUrl."'><i class='fa-manage-cost' style='height:22px;width:23px;'></i></a>";
		...
		$wetip = str_replace('"', "", $wetip);
			...
			//$wetip =htmlspecialchars($wetip ,ENT_QUOTES, "UTF-8");
		...
		if( $project_level == 1) {
			$tip .= "<span style='border-bottom:1px solid #ccc; margin-top:5px !important; margin-bottom:5px !important;'></span><span class='wpbudget'>Budget (".$currency_sign."): ". number_format($workspacEstimateCost,2, '.', '')."</span><span class='wpbudget'>Actual (".$currency_sign."): ". number_format($workspacSpendCost,2, '.', '')."</span><span style='border-bottom:1px solid #ccc; margin-top:5px !important; margin-bottom:5px !important;'></span>";
		...
		if(isset($all_tasks) && !empty($all_tasks)){
			...
			foreach ($all_element_data as $task_key => $task_val) {
				...
				$element_estimate_cost = ( isset($otdata['ele_estimated_cost']) && !empty($otdata['ele_estimated_cost']) ) ? $otdata['ele_estimated_cost'] : 0;
				$element_actual_cost = ( isset($otdata['ele_actual_cost']) && !empty($otdata['ele_actual_cost']) ) ? $otdata['ele_actual_cost'] : 0;
				...
				if( $project_level == 1) {
					$task_html = "<a class='perm' href='".$costUrl."'><i class='fa-manage-cost' style='height:22px;width:23px;'></i></a>";
...
if(isset($wsp_tasks_data_filter) && !empty($wsp_tasks_data_filter)){
	foreach ($wsp_tasks_data_filter as $key => $value) {
		...
		if( $project_level == 1) {
			$tip .= "<span style='border-bottom:1px solid #ccc; margin-top:5px !important; margin-bottom:5px !important;'></span><span class='wpbudget'>Budget (".$projectCurrencyName."): ". number_format($workspacEstimateCost,2, '.', '')."</span><span class='wpbudget'>Actual (".$projectCurrencyName."): ". number_format($workspacSpendCost,2, '.', '')."</span><span style='border-bottom:1px solid #ccc; margin-top:5px !important; margin-bottom:5px !important;'></span>";
			$tip .= "<span class='wpbudget'>Task Risks: ".$severe_risk." SEVERE, ".$high_risk." HIGH</span>";
			$tip .= "</div></div>";
		...
		if(isset($all_tasks) && !empty($all_tasks)){
			...
			foreach ($all_element_data as $task_key => $task_val) {
				...
				if( $project_level == 1) {
					$etip_html .= "<span style='border-bottom:1px solid #ccc; margin-top:5px !important; margin-bottom:5px !important;'></span><span class='wpbudget'>Budget (".$projectCurrencyName."): ". number_format($element_estimate_cost,2, '.', '')."</span><span class='wpbudget'>Actual (".$projectCurrencyName."): ". number_format($element_actual_cost,2, '.', '')."</span><span style='border-bottom:1px solid #ccc; margin-top:5px !important; margin-bottom:5px !important;'></span>";
                    ...
                    <div class="pull-left" style="">
                        ...
                        <span class="completeddays">
							...
							<?php
							 ...
							 $projectSpendTotal =  (isset($projectSpendTotal) && !empty($projectSpendTotal)) ? number_format($projectSpendTotal, 2, '.', '') : 0.00;

							$project_cost_icons = "<a class='perm' href='".$costUrl."'><i class='fa-manage-cost' style='height:22px;width:23px;'></i></a>";
							...
							<span class='wpbudget'>Budget (".$projectCurrencyName."): ". (isset($project_detail['budget']) && !empty($project_detail['budget'])? $project_detail['budget'] : 0.00 )."</span>
							<span class='wpbudget'>Actual (".$projectCurrencyName."): ". $projectSpendTotal."</span></div></div> ";
