...
/*if( isset($projectCurrencyName) && !empty($projectCurrencyName) ){
	$currency_symbol = $crysymbol = $projectCurrencyName;
}*/
		...
		if(isset($all_tasks) && !empty($all_tasks)){
			$all_element_data = $this->Permission->task_detail_filter($all_tasks,$status,$criticalStatus,$assignUserId);
...
<style type="text/css">
	...
	.dependencyblack{ height:23px; width:23px;background-repeat: no-repeat;
