<?php
$skillData['title'] = $data[0]['subjects']['title'];
$skillData['id'] = $data[0]['subjects']['id'];
$skillData['description'] = $data[0]['subjects']['description'];
$skillData['modified'] = $data[0]['subjects']['modified'];
$skillData['modified_by'] = $data[0]['subjects']['modified_by'];
$skillData['image'] = $data[0]['subjects']['image'];
...
$skillData['skill_links'] = json_decode($data[0][0]['subject_links'],true);
$skillData['skill_files'] = json_decode($data[0][0]['subject_files'], true);
$skillData['user_detail'] = json_decode($data[0][0]['user_detail'], true);
$skillData['location_subject'] = json_decode($data[0][0]['location_subject'], true);
...
$all_dept = (isset($data[0][0]['department_sub']) && !empty($data[0][0]['department_sub'])) ? json_decode($data[0][0]['department_sub'], true) : [];
...
?>
			<div class="modal-header">
				...
				<h3 class="modal-title"><i class="subjectwhiteicon"></i> <span class="text-ellipsis pl9"><?php echo $item_title;?></span></h3>
			</div>
			<div class="modal-body">
				<div class="row d-flex-s">

				<div class="col-sm-3 col-md-3 col-lg-3 col-skill-1 skill-profile-cols skill-subject-domain-left">
					<div class="skill-profile-img subjects-orange-border">
					<?php if( isset($skillData['image']) && !empty($skillData['image']) ){
							if( file_exists(WWW_ROOT.SUBJECT_IMAGE_PATH.$skillData['image']) ){
						?>
						<img class="" alt="profile" src="<?php echo SITEURL.SUBJECT_IMAGE_PATH.$skillData['image'];?>?time=<?php echo time();?>">
					<?php
						} else {
							echo "No Image";
						}
					...
					<ul>
						...
						if( isset($skillData['totalpeople']) && !empty($skillData['totalpeople']) ){
							$sk_link = Router::Url( array( "controller" => "searches", "action" => "people", "subject" => $skillData['id'], 'admin' => FALSE ), true );
						...
						<div class="tab-content skills-tab-content">
							...
							<div id="files" class="tab-pane fade">
								<div class="popup-skill-list">
										...
										?>
										  <li><span class="list-text"><?php echo htmlentities($listfiles['file_name'], ENT_QUOTES, "UTF-8");?></span> <span class="list-icon"> <a href="<?php echo SITEURL?>competencies/download_files/<?php echo $listfiles['id']; ?>/subject" class="tipText" title="Download File" download><i class="downloadblackicon"></i></a>  </span></li>
										<?php }
										} else {
											echo '<li>No Files</li>';
										}
								...
								<div id="community" class="tab-pane fade">
									<div class="popuplocationlist" style="overflow: hidden; width: auto; height: 312px;">
										<ul class="loc-names-ul">
											...
											<?php } ?>
											<?php if( isset($skillData['location_subject']) && !empty($skillData['location_subject']) ){
												$loc_ids = Set::extract($skillData['location_subject'], '{n}/id');
...
$(function(){
	...
	$('body').on('click', '.filter-selected', function(event) {
		...
		if( window.location.href.indexOf("competencies") <= 0 ) {
			...
			window.location.href = $js_config.base_url+'competencies/index/subjects/'+id;
		}
