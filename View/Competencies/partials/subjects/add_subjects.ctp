<?php
	$buttonText = 'Add';
	$titleText = 'Add';
	$this->request->data['Subject']['title'] = '';
	$this->request->data['Subject']['description'] = '';
	$this->request->data['Subject']['image'] = '';
	$this->request->data['Subject']['id'] = '';

	if( isset($subject_data) && !empty($subject_data) ){
		$buttonText = 'Save';
		$titleText = 'Edit';
		$this->request->data['Subject']['title'] = $subject_data['Subject']['title'];
		$this->request->data['Subject']['description'] = $subject_data['Subject']['description'];
		$this->request->data['Subject']['image'] = $subject_data['Subject']['image'];
		$this->request->data['Subject']['id'] = $subject_data['Subject']['id'];
...
?>
			<div class="modal-header">
				...
				<h4 class="modal-title"><?php echo $titleText;?> Subject</h4>
			</div>
			<div class="modal-body">
				<ul class="nav nav-tabs" id="add_tabs">
					...
					<li class="">
						<a data-toggle="tab" data-tab="subject_keyword" data-type="keywords" id="keywordTab" data-target="#tab_keywords" href="#tab_keywords" aria-expanded="false" class="skilltab tab-file subject_keyword" >Keywords</a>
					</li>
							...
							<div class="row">
									...
									<div class="form-group">
									  ...
									  <div class="col-lg-10">
										  <div class="form-control-skill">
										  <div class="title-group-w">
										<?php echo $this->Form->input('SubjectFile.file_name', array('type' => 'text', 'label' => false, 'div' => false, 'class' => 'form-control', 'autocomplete'=>'off')); ?>
			...
			<div class="modal-footer clearfix">
				...
				<button type="button" class="btn btn-primary submit_subject submit_data" ><?php echo $buttonText;?></button>
...
$(function(){
	...
	$('input[name="data[Subject][title]"]').focus();
	...
	$('.submit_keywords').off('click').on('click', function(event) {
		...
		$("[name='data[Keyword][keyword]']").focus();
	});
		...
		$.ajax({
			...
			success: function(response){
				if(response.success){
					...
					$.temp_detail_file = '';
					$.tempfile = '';
					$('.search-box[data-type="subjects"]').val('').trigger('keyup');
	...
	$('.remove-image').off('click').on('click', function(event) {
		...
		if($(this).parents('.input-group').find('input[type="file"]').is($('#subject_image'))) {
