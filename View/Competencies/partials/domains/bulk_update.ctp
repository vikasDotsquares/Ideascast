	...
	<div class="content" style="min-height:0;">
		 <div class="row">
			<div class="form-group item-selection">
				  <label for="UserUser" class="control-label">Domains:</label>
				  <?php $domains = htmlentity($domains);
	...
	$(function(){
		...
		$('.bulkupdate').off('click').on('click', function(e){
			...
			if(selected_ids){
				...
				$("#modal_confirm").find('input[name="selected_type"]').val('Domain');
				setTimeout(function(){
					$("#modal_confirm").find('.selected-type-text').text('Domains');
				}, 1000)
				$("#modal_confirm").modal('show');
			}
			else{
				$('.item-selection .error-text').text('Please select at least one domain');
			}
		})
