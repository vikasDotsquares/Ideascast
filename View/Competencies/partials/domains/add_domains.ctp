<?php
	$buttonText = 'Add';
	$titleText = 'Add';
	$this->request->data['KnowledgeDomain']['title'] = '';
	$this->request->data['KnowledgeDomain']['description'] = '';
	$this->request->data['KnowledgeDomain']['image'] = '';
	$this->request->data['KnowledgeDomain']['id'] = '';

	if( isset($domain_data) && !empty($domain_data) ){
		$buttonText = 'Save';
		$titleText = 'Edit';
		$this->request->data['KnowledgeDomain']['title'] = $domain_data['KnowledgeDomain']['title'];
		$this->request->data['KnowledgeDomain']['description'] = $domain_data['KnowledgeDomain']['description'];
		$this->request->data['KnowledgeDomain']['image'] = $domain_data['KnowledgeDomain']['image'];
		$this->request->data['KnowledgeDomain']['id'] = $domain_data['KnowledgeDomain']['id'];
...
?>
			<div class="modal-header">
				...
				<h4 class="modal-title"><?php echo $titleText;?> Domain</h4>
			</div>
			...
			<div class="modal-footer clearfix">
				...
				<button type="button" class="btn btn-primary submit_domain submit_data" ><?php echo $buttonText;?></button>
...
$(function(){
	...
	$('input[name="data[KnowledgeDomain][title]"]').focus();
		...
		$.ajax({
			...
			success: function(response){
				if(response.success){
					...
					$.temp_detail_file = '';
					$.tempfile = '';
					$('.search-box[data-type="domains"]').val('').trigger('keyup');
				...
				$.ajax({
					...
					success:function(response){
						if(response.success){
							...
							setTimeout(function(){
								$form.parents(".add-skill-t").find(".submit_domain").prop('disabled', false);
							},250);
