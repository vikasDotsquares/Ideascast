<?php
// pr($data);
$skillData['id'] = $data[0]['knowledge_domains']['id'];
//$skillData['title'] = $data[0]['knowledge_domains']['title'];
$skillData['title'] = $data[0]['knowledge_domains']['title'] ;
$skillData['description'] = $data[0]['knowledge_domains']['description'];
$skillData['modified'] = $data[0]['knowledge_domains']['modified'];
$skillData['modified_by'] = $data[0]['knowledge_domains']['modified_by'];
$skillData['image'] = $data[0]['knowledge_domains']['image'];
...
$skillData['domain_links'] = json_decode($data[0][0]['domain_links'],true);
$skillData['domain_files'] = json_decode($data[0][0]['domain_files'], true);

if(isset($skillData['domain_links']) && !empty($skillData['domain_links'])){
	usort($skillData['domain_links'], function($a, $b) {
	    $t1 = iconv('UTF-8', 'ASCII//TRANSLIT', $a['link_name']);
		$t2 = iconv('UTF-8', 'ASCII//TRANSLIT', $b['link_name']);
	    return strcasecmp($t1, $t2);
	});
}
if(isset($skillData['domain_files']) && !empty($skillData['domain_files'])){
	usort($skillData['domain_files'], function($a, $b) {
...
$skillData['location_domain'] = json_decode($data[0][0]['location_domain'], true);
...
$all_dept = (isset($data[0][0]['department_domain']) && !empty($data[0][0]['department_domain'])) ? json_decode($data[0][0]['department_domain'], true) : [];
...
?>
			<div class="modal-header">
				...
				<h3 class="modal-title"><i class="domainwhiteicon"></i> <span class="text-ellipsis pl6"><?php echo $item_title;?></span></h3>
			</div>
			<div class="modal-body">
				<div class="row d-flex-s">

				<div class="col-sm-3 col-md-3 col-lg-3 col-skill-1 skill-profile-cols skill-subject-domain-left">
					<div class="skill-profile-img domains-green-border">
					<?php if( isset($skillData['image']) && !empty($skillData['image']) ){
							if( file_exists(WWW_ROOT.DOMAIN_IMAGE_PATH.$skillData['image']) ){
					?>
						<img class="" alt="profile" src="<?php echo SITEURL.DOMAIN_IMAGE_PATH.$skillData['image'];?>?time=<?php echo time();?>">
					<?php
						} else {
							echo "No Image";
						}
					...
					<ul>
						...
						if( isset($skillData['totalpeople']) && !empty($skillData['totalpeople']) ){
							$sk_link = Router::Url( array( "controller" => "searches", "action" => "people", "domain" => $skillData['id'], 'admin' => FALSE ), true );
						...
						<div class="tab-content skills-tab-content">
							...
							<div id="links" class="tab-pane fade">
								<div class="popup-skill-list">
									<ul>
										<?php if( isset($skillData['domain_links']) && !empty($skillData['domain_links']) ){
											foreach($skillData['domain_links'] as $listlinks){
							...
							<div id="files" class="tab-pane fade">
								<div class="popup-skill-list">
										<ul>
										  <?php if( isset($skillData['domain_files']) && !empty($skillData['domain_files']) ){
											foreach($skillData['domain_files'] as $listfiles){
										?>
										  <li><span class="list-text"><?php echo htmlentities($listfiles['file_name'], ENT_QUOTES, "UTF-8");?></span> <span class="list-icon"> <a href="<?php echo SITEURL?>competencies/download_files/<?php echo $listfiles['id']; ?>/domain" class="tipText" title="Download File" download><i class="downloadblackicon"></i></a>  </span></li>
										<?php }
										} else {
											echo '<li>No Files</li>';
										}
							...
							<div class="tab-content">
								<div id="people" class="tab-pane fade active in">
									...
									<li><div class="community-diff-list">
										<span class="skill-popple-icon" data-toggle="modal" data-target="#popup_modal" data-remote="<?php echo Router::Url( array( "controller" => "shares", "action" => "show_profile", $listuser['user_id'], 'admin' => FALSE ), true ); ?>"><img class="" alt="<?php echo $profilesPic; ?>" src="<?php echo $profilesPic; ?>"></span>
								...
								<div id="community" class="tab-pane fade">
									<div class="popuplocationlist" style="overflow: hidden; width: auto; height: 312px;">
										<ul class="loc-names-ul">
											...
											<?php } ?>
										<?php if( isset($skillData['location_domain']) && !empty($skillData['location_domain']) ){
											$loc_ids = Set::extract($skillData['location_domain'], '{n}/id');
...
$(function(){
	...
	$('body').on('click', '.filter-selected', function(event) {
		...
		if( window.location.href.indexOf("competencies") <= 0 ) {
			...
			window.location.href = $js_config.base_url+'competencies/index/domains/'+id;
		}
