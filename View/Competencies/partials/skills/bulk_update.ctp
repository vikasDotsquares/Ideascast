	...
	<div class="content" style="min-height:0;">
		 <div class="row">
			<div class="form-group item-selection">
				  <label for="UserUser" class="control-label">Skills:</label>
				  <?php $skills = htmlentity($skills);
...
<script>
	$(function(){
		var $bulk_update =  $('#bulk_update').multiselect({
		...
		$('body').delegate('.bulkupdate',  'click', function(e){
			...
			if(selected_ids){
				...
				$("#modal_confirm").find('input[name="selected_type"]').val('Skill');
				setTimeout(function(){
					$("#modal_confirm").find('.selected-type-text').text('Skills');
				}, 1000)

				$("#modal_confirm").modal('show');
			}
			else{
				$('.item-selection .error-text').text('Please select at least one skill');
			}
		})
	})
