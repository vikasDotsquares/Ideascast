...
if(isset($permit_data['pp_data']) && !empty($permit_data['pp_data'])) {
 ...
 //if(isset($User['id'])){
		...
		//$projectList = $this->Common->ProjectBoardProjectUserList($Project['id']);
		 $projectList = $this->Common->ProjectBoardProjectUserListNew($Project['id']);
		//echo $Project['id'];
		//pr($projectList);
...
<div class="style-people-com requestfromtitle">
					<span class="style-popple-icon-out">
								...
								<td class="text-center project-requests-but">
									...
									?>
										<a class="btn btn-success btn-sm request_accept tipText" data-project="<?php echo $Project['id'];?>"  data-projectboard="<?php echo $UserPlist['ProjectBoard']['id'];?>"  data-sender="<?php echo $requestSenderID;?>"  title="Share" data-redirect="<?php echo Router::Url(array('controller' => 'shares', 'action' => 'index', $Project['id'],$requestSenderID, 1, 'admin' => FALSE ), TRUE); ?>" >
									...
									$project_permit = $this->Common->project_permission_details($Project['id'],$requestSenderID );
										...
										echo ( isset($updatedDate['ProjectBoard']['updated']) && !empty($updatedDate['ProjectBoard']['updated'])) ? $this->Wiki->_displayDate($date = date('Y-m-d h:i:s A',strtotime($updatedDate['ProjectBoard']['updated'])),$format = 'd M, Y h:i A') : 'N/A'.'</span>';
										?>
...
$(function () {
	...
	$(".request_accept").on('click', function(){
		$that = $(this);
		var redirect_location = $that.data('redirect');
		var projectBoard = $that.data('projectboard');
		var sender = $that.data('sender');
		
		if( projectBoard ){
			$.ajax({
				url: $js_config.base_url + 'boards/project_accept',
				type: "POST",
				data: $.param({id: projectBoard}),
				dataType: "JSON",
				global: false,
