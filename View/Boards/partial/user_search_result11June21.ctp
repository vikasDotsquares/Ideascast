...
if(isset($pp_data_users) && !empty($pp_data_users)) {
 ...
 //if(isset($User['id'])){
		...
		$projectList = $this->Common->ProjectBoardProjectList($Board['receiver'],$Board['sender']);
							...
							<tr class=" ">
								...
								<td class="text-center project-requests-but">
									...
									$project_permit = $this->Common->project_permission_details($UserPlist['ProjectBoard']['project_id'],$requestSenderID );
										...
										if( isset($Board['updated']) && !empty($Board['updated']) ){
											echo '<span class="small"><strong>Accepted</strong><br>';
											echo $this->Wiki->_displayDate($date = date('Y-m-d h:i:s A',strtotime($Board['updated'])),$format = 'd M, Y h:iA').'</span>';
										} else {
