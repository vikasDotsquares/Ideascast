...
<div class="modal-header">
	<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
		<h3 class="modal-title" id="myModalLabel" style="text-overflow: ellipsis;
    display: inline-block;
    overflow: hidden;
    white-space: nowrap;
    width: 90%;"><?php echo $project_title; ?>
	</h3>
</div>
...
if( isset($pdata) && !empty($pdata) ) {
	...
	<li class="has-sub-cat">
		<a  class="tipText tree-toggler nav-header tree_links"> <i class="<?php echo $icon_class; ?>"></i>
			<span class="tree_text tipText" title="Project" style=""><?php echo strip_tags($project_title); ?></span>
		</a>
		...
		foreach($wpdata as $k => $v) {
			...
			echo  '<li class="has-sub-cat">';
				echo  '<a ' .
					...
					'<span class="tree_text tipText" title="Workspace" style="">'.strip_tags($wsDetail['Workspace']['title']).'</span>' .
					'</a>' ;
			...
			if( !empty($edata) ) {
				...
				foreach($edata as $k => $v) {
					...
					echo  '<a ' .
						...
						'<span class="tree_text tipText" title="Task" style="">'.strip_tags($elDetail['Element']['title']).'</span>' .
						'</a>' ;
					echo '<div class="sharing-icon"> ';
