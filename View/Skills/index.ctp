	...
	<div class="col-xs-12">
		...
		<div class="row">
			<section class="content-header clearfix">
                <h1 class="pull-left"><?php   echo $page_heading; ?> (<?php echo $this->Common->totalDataS('Skill'); ?>)
            ...
            <div class="col-xs-12">
				<div class="box no-box-shadow box-success">
					<div class="box-header">
						...
						<?php $editURL = SITEURL."skills/skill_edit/";

							$url = array('controller' => 'skills','action' => 'index');
							if( $this->params['paging']['Skill']['page'] > 1 ){
								$my_params = array(
						...
						?>
							<a class="btn btn-primary edituser" data-toggle="modal" data-target="#modal_box" data-remote="<?php echo SITEURL."skills/manage_skill" ; ?>" >
								Add
						...
						<div class="<?php echo $class; ?> search pull-right" style="width: 100%; margin-top: 10px;" id="collapseExample">
							<div class="well">
								...
								if(isset($this->params['named']['sortorder']) && !empty($this->params['named']['sortorder'])){
									$formAction = SITEURL."skills/index/page:".$this->params['paging']['Skill']['page']."/sortorder:".$this->params['named']['sortorder'];
								} else {
									$formAction = SITEURL."skills/";
								}

								//echo $formAction; exit;

								echo $this->Form->create('Skill', array( 'url' => $formAction, 'type' => 'POST', 'class' => 'form-horizontal form-bordered', 'id' => 'search_page_show_form')); ?>
									<div class="modal-body">
										<div class="form-group">
											...
											<div class="col-xs-8 col-sm-8 col-md-8 col-lg-3" style="padding-right: 0">
												<?php
												echo $this->Form->input('keyword', array('placeholder' => 'Search for Skill…','type' => 'text','label' => false, 'value'=>$keyword,'div' => false, 'class' => 'form-control')); ?>
											</div>
											...
											<div   style="text-align:right;float:left;">
												...
												<a class="btn btn-primary searchbtn" href="<?php echo SITEURL; ?>skills/skill_resetfilter" >Close</a>
											</div>
										</div>
									</div>
						...
						<table id="example2" class="table table-bordered table-hover">
							<thead>
								<tr>
									...
									<th><?php echo $this->Paginator->sort('Skill.title',__("Skill"));?></th>
							...
							<tbody id="tbody_skills">
								...
								<tr <?php if($this->params['paging']['Skill']['page']==1 && $icount == 0){ echo 'data-first="true"'; }?> <?php if($this->params['paging']['Skill']['page']==$this->params['paging']['Skill']['pageCount'] && $icount == (count($currencies)-1) ){ echo 'data-last="true"'; }?>  data-id="<?php echo $currency['Skill']['id']; ?>" data-order="<?php echo $currency['Skill']['sorder']; ?>">
									...
									<td><?php echo $currency['Skill']['title'];//."==".$currency['Skill']['sorder']; ?>
									</td>
									<td>
										<?php
									...
									<td>
										...
										<a data-toggle="modal" class="edituser tipText" data-target="#modal_box" title="Edit Skill" data-remote="<?php echo $editURL; ?>"  data-tooltip="tooltip" data-placement="top" style="cursor:pointer;" ><i class="fa fa-fw fa-edit"></i></a>

										<?php
											//$deleteURL = SITEURL."skills/skill_delete/".$currency['Skill']['id'];
											$deleteURL = SITEURL."skills/skill_delete/";
										?>
										<a class="RecordDeleteClass tipText" rel="<?php echo $currency['Skill']['id']; ?>" title="Delete Skill" data-whatever="<?php echo $deleteURL; ?>"  data-tooltip="tooltip" data-placement="top" style="cursor:pointer;"><i class="fa fa-fw fa-trash-o"></i></a>
									</td>
								</tr>
								<?php
										$num++;
								...
								<?php } else { ?>
								<tr>
                                    <td colspan="6" style="color:RED;text-align: center;">No Skills Found.</td>
								</tr>
                                    <?php
										}
									?>
...
$(function(){
	...
	$( "#example2sssss tbody" ).sortable({
		...
		update: function(event, ui) {
			...
			$.ajax({
				url : '<?php echo SITEURL."skills/skill_DragDrop/";?>',
				async:false,
				global: false,
				type:"POST",
				data:$.param({skill_id:order,skill_order:allCheckbox,sorder:sortOrder}),
...
$(".skillMoveUp").click(function(){
	...
	if( chklgth > 1 ){
		msgchk = 'You can not move multiple skill at a time.';
	} else if( chklgth < 1 ) {
		msgchk = 'Please select atleast one skill.';
	} else {

		$.ajax({
		url : '<?php echo SITEURL."skills/skill_swap/";?>',
		async:false,
		global: true,
		type:"POST",
		data:$.param({skill_id:allCheckbox, moveto:movetos,movetoid:checkedBox,sortorder:sortOrder}),
		success:function(response){
...
$(".skillMoveDown").click(function(){
	...
	$(document).on('click', '.RecordUpdateClass5', function(){
		...
		$('#RecordStatusFormId').attr('action', '<?php echo SITEURL; ?>skills/skill_updatestatus');
		$('#recordID').val(id);
		if(rel == 'activate'){
			$('#recordStatus').val(1);
		}else{
	...
	$(document).on('click', '.RecordUpdateClass', function(event){
		...
		event.preventDefault();
			...
			var deleteURL = '<?php echo SITEURL; ?>skills/skill_updatestatus';
...
$(function(){
					...
					action: function (dialogRef) {
						$.when(
							$.ajax({
								url : deleteURL,
								type: "POST",
								data: $.param({skill_id:deleteid,deltype:'single'}),
								global: true,
								async:false,
								success:function(response){
		...
		$(".deletemultiple").click(function(event){
			...
			if( allChecked.length > 0 ){
				...
				BootstrapDialog.show({
				title: 'Confirmation',
				message: 'Are you sure you want to remove the selected Skills?',
				type: BootstrapDialog.TYPE_DANGER,
				draggable: true,
				buttons: [
				{
						...
						action: function (dialogRef) {
							$.when(
								$.ajax({
									...
									type: "POST",
									data: $.param({skill_id:allChecked,deltype:'multiple'}),
									global: true,
									async:false,
									success:function(response){
			...
			/*if( allChecked.length > 0 ){
				...
				url : deleteURL = '<?php echo SITEURL."skills/skill_delete/";?>',
				type: "POST",
				data: $.param({skill_id:allChecked}),
				global: true,
				async:false,
				success:function(response){
