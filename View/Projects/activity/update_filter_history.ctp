       ...
       // pr($history_list);
        ...
        <div class="row ">
            <div class="col-sm-1 history_gap col-activity1">
                ...
                $html = '';
                if(  $history_list['ProjectActivity']['updated_user_id'] != $current_user_id ) {
                        $html = CHATHTML($history_list['ProjectActivity']['updated_user_id']);
                }
                //echo SITEURL.'img/image_placeholders/logo_placeholder.gif';
                ?>
                <img data-remote="<?php echo SITEURL ?>shares/show_profile/<?php echo $history_list['ProjectActivity']['updated_user_id'] ?>"  data-target="#popup_modal"  data-toggle="modal" class="thumbnail-history pophover" align="left" data-content="<div><p><?php echo htmlentities($history_list['UserDetail']['first_name'],ENT_QUOTES) . ' ' .htmlentities($history_list['UserDetail']['last_name'],ENT_QUOTES); ?></p><p><?php echo htmlentities($job_title,ENT_QUOTES); ?></p><?php echo $html; ?></div>"  src="<?php echo $urlImg; ?>"  src="<?php echo $urlImg; ?>" style="width:50px;"> 
            </div>
            <div class="col-sm-1 text-left history_gap col-activity2">
                <?php
                $user_type_per = $this->Common->project_permission_details($history_list['ProjectActivity']['project_id'], $history_list['ProjectActivity']['updated_user_id']);
                $user_type = $this->Common->userproject($history_list['ProjectActivity']['project_id'], $history_list['ProjectActivity']['updated_user_id']);
            ...
            <div class="col-sm-3 text-left history_gap col-activity3">
                <a href="#" style="float: none;" data-remote="<?php echo SITEURL ?>shares/show_profile/<?php echo $history_list['ProjectActivity']['updated_user_id'] ?>"  data-target="#popup_modal"  data-toggle="modal" class="view_profile text-maroon" >
            ...
            <div class="col-sm-4 col-activity4">
                <span><?php echo $history_list['ProjectActivity']['message']; ?>,</span><span>    
                <?php 
				// pr($history_list['Activity']);
				//echo date('d M, Y g:iA', strtotime($history_list['ProjectActivity']['updated'])); 
				echo $this->Wiki->_displayDate($date = date('Y-m-d h:i:s A',strtotime($history_list['ProjectActivity']['updated'])),$format = 'd M, Y g:iA'); 
				?></span>
            </div>
            <div class="col-sm-3 text-left history_gap col-activity5">
                <?php
                App::uses('CakeTime', 'Utility');
                $date = strtotime($history_list['ProjectActivity']['updated']);
                echo '<i class="fa fa-clock-o"></i> ';
                // echo CakeTime::timeAgoInWords_custom($date, array('format' => 'F jS, Y', 'end' => '+10 year'));
				echo time_elapsed($history_list['ProjectActivity']['updated'], true);
                ?>
            </div>
<!--            <div class="col-sm-1 history_gap history_gap_del">
                <a data-remote="<?php echo SITEURL; ?>entities/delete_history/<?php echo $history_list['ProjectActivity']['id']; ?>" data-auth-remote="" data-header="Authentication" data-toggle="confirmation" data-msg="Are you sure you want to delete this history?" title="" data-action="remove" data-id="<?php echo $history_list['Activity']['id']; ?>" class="btn btn-sm btn-danger remove_history tipText" href="#" data-original-title="Remove">
