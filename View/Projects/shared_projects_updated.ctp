...
<div id="list_grid_container"  style="border: 1px solid #ccc;">
	...
	<ul class="grid clearfix" id="ul_list_grid">
		...
		foreach( $projects as $key => $val ) {
			...
			$open_project_link =  SITEURL.'projects/index/'.$item['id'];
									...
									$grp_id = $this->Group->GroupIDbyUserID($item['id'],$user_id);

									if(isset($grp_id) && !empty($grp_id)){

									$group_permission = $this->Group->group_permission_details($item['id'],$grp_id);
		...
		<li class="li-listing">
			<div class="panel <?php echo $item['color_code'] ?>">
				...
				<div class="panel-heading" style="position: relative">
					<h4 class="panel-title" title="<?php echo strip_tags($item['title']); ?>" style="display: inline-block; width: 95%; max-width:100%;">
						<!-- <a href="<?php echo $open_project_link; ?>" class="project_title" data-full-title="<?php echo strip_tags($item['title']); ?>"> -->
							<span class="p-list-h tipText" title="<?php echo strip_tags($item['title']); ?>" style="float: none;"><?php echo strip_tags( $item['title'] ); ?> </span>
						<!-- </a> -->
					...
					<div class="main-content" style="">
						...
						<div class="right-content" style="">
							<div class="text-content" style="">
								...
								<h5 style="" class="text-black sub-heading padding-sm">Tasks <span class="hidden-md">(Next 14 days and Overdue)</span>
						...
						<?php
							...
							<ul class="list-unstyled">
								<?php
									...
									$e_permission = $this->Common->element_permission_details($ws_id ,$item['id'],$this->Session->read('Auth.User.id'));
									...
									if((isset($grp_id) && !empty($grp_id))){
											...
											$e_permission =  $this->Group->group_element_permission_details( $ws_id, $item['id'], $grp_id);
											}


									}

									if( (((isset($user_project) && !empty($user_project)) || (isset($project_level) && $project_level==1)   ||  (isset($p_permission['ProjectPermission']['project_level']) && $p_permission['ProjectPermission']['project_level'] == 1)) )   || (isset($e_permission) && !empty($e_permission) && in_array($element['id'],$e_permission))){
