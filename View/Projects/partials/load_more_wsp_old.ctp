				...
				if(( ((isset($wwsid) && !empty($wwsid))) &&  (in_array($project_workspace_data['id'], $wwsid)))  || ((isset($user_project)) && (!empty($user_project))) || (isset($project_level) && $project_level==1) ||  (isset($p_permission['ProjectPermission']['project_level'])  && $p_permission['ProjectPermission']['project_level'] ==1 )   ) {
					...
					if (isset($workspaceArray['id']) && !empty($workspaceArray['id'])) {
						...
						if (isset($workspace_areas) && !empty($workspace_areas)) {
							...
							$progress_data = $this->ViewModel->countAreaElements($workspaceArray['id'], null, null, $project_id, $user_project['UserProject']['id'], $user_project, $p_permission);
						...
						?>
							<tr id="<?php echo $project_workspace_data['id']; ?>" data-value="<?php echo $workspaceArray['id']; ?>" data-id="<?php echo $workspaceArray['id']; ?>" data-pid="<?php echo $project_id; ?>">
								<td>
									...
									<div class="small-box panel <?php echo $class_name ?>">
										<a class="inner" href="<?php echo $create_elements_link; ?>">
											<strong class="ellipsis-word tipText" style="text-transform:none !important" title="<?php echo strip_tags($workspaceArray['title']); ?>"><?php echo strip_tags($workspaceArray['title']) ;  ?></strong>
											<?php
								...
								<td style="vertical-align: top ! important; font-size: 12px; line-height: 16px; " class=" ">
									<div style="max-height: 65px;  overflow: hidden;" class="key_target" data-placement="top" data-content="<div class='template_create'><?php echo nl2br(htmlentities($workspaceArray['description'], ENT_QUOTES)) ; ?></div> "  data-toggle="popover" data-trigger="hover">
										<?php echo  nl2br(htmlentities($workspaceArray['description'])) ; ?>
									</div>
								</td>
										...
										<ul class="list-unstyled">
											...
											<li>
												<span class="label bg-mix">
													<?php   echo  $progress_data['overdue_element_count']; ?>
												</span>
												<span class="btn btn-xs bg-element no-change tipText" title="<?php echo tipText('Overdue Statuses') ?>"  href="#"><i class="fa fa-exclamation"></i></span>
											</li>
										</ul>
									</span>
								</td>
								   	...
								   	<span class="text-center el-icons">
										<ul class="list-unstyled">
										 	<li>
											  ...
											  <span class="btn btn-xs bg-maroon no-change tipText" title="<?php echo tipText('Links') ?>"  href="#"><i class="fa fa-link"></i></span>
										 	</li>
										 	<li>
											  <span class="label bg-mix">
												<?php
											  ...
											  <span class="btn btn-xs bg-purple no-purple tipText" title="<?php echo tipText('Notes') ?>"  href="#"><i class="fa fa-file-text-o"></i></span>
										 	</li>
										 	<li>
											  <span class="label bg-mix">
												<?php
											  ...
											  <span class="btn btn-xs bg-blue no-change tipText" title="<?php echo tipText('Documents') ?>"  href="#"><i class="fa fa-folder-o"></i></span>
										 	</li>
										 	...
										 	<li>
											  ...
											  <span class="btn btn-xs bg-green no-change tipText" title="<?php echo tipText('Mind Maps') ?>"  href="#"><i class="fa fa-sitemap"></i></span>
										 	</li>


									 		<li>
											  <span class="label bg-mix"><?php echo show_counters($workspaceArray['id'], 'decision'); ?></span>
											  <span class="btn btn-xs bg-orange no-change tipText" title="<?php echo tipText('Live Decisions') ?>"  href="#"><i class="far fa-arrow-alt-circle-right"></i></span>
										 	</li>
										 	<li>
											  	...
											  	?></span>
											  	<span class="btn btn-xs bg-teal no-change tipText" title="<?php echo tipText('Live Feedbacks') ?>"  href="#"><i class="fa fa-bullhorn"></i></span>
										 	</li>
										 	...
										 	<li>
											  ...
											  <span class="btn btn-xs bg-yellow no-change tipText" title="<?php echo tipText('Live Votes') ?>"  href="#"><i class="fa fa-inbox"></i></span>
										 	</li>
										</ul>
									</span>
								</td>
								<td class="action-buttons" style="text-align: center ! important;">
									...
									<div class="btn-group btn-actions">
										...
										<a data-toggle="modal" data-target="#modal_delete" data-remote="<?php echo Router::Url( array( "controller" => "workspaces", "action" => "delete_an_item", $project_id, $workspaceArray['id'], $project_workspace_data['id'], 'admin' => FALSE ), true ); ?>" type="button" class="btn btn-xs tipText delete-an-item <?php echo $class_name ?>" title="Move to Bin">
