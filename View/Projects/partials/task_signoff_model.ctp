...
<script type="text/javascript">
	$(function() {
		...
		$('body').delegate('#signofffrm', 'submit', function(e){
			...
			form_data.append('data[Project][id]', project_id);
			form_data.append('data[Project][sign_off]', sign_off);
			...
			//$(".submit_signoff").find('i').show();
			//console.log($(".submit_signoff").find('i'));
			//return;
			...
			if( signoff_comment.length > 0 ){
				...
				$.ajax({
					url: $js_config.base_url+'projects/save_signoff_comment',
					type: "POST",
					cache: false,
					contentType: false,
					processData: false,
