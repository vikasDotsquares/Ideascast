		...
		<section class="content-header clearfix">
			<h1 class="pull-left">
				...
				<p class="text-muted date-time">
					<span>Created: <?php echo date('d M Y h:i:s', $project_detail['Project']['created']); ?></span>
					<span>Updated: <?php echo date('d M Y h:i:s', $project_detail['Project']['modified']); ?></span>
				</p>
					<?php }
				else {
							...
							if( $leftbar_status ) {
								...
								if( isset($workspaceArray['id']) && !empty($workspaceArray['id'])) {
									...
									if( isset($workspaceArray['Area']) && !empty($workspaceArray['Area']) ) {
										...
										$areasCnt = ( isset($areas) && !empty($areas) ) ? count($areas) : 0;
										
										if ( $totalValidAreas = $areasCnt ) {
...
<td >
	<div class="small-box panel <?php echo $class_name ?>">
		...
		<a class="inner" href="<?php echo $create_elements_link; ?>">
			...
			<span class="text-muted date-time">
				<span>Created:
				<?php echo ( isset($workspaceArray['created']) && !empty($workspaceArray['created'])) ? date('d M Y', strtotime($workspaceArray['created'])) : 'N/A'; ?></span>
				<span>Updated:
				<?php echo  ( isset($workspaceArray['modified']) && !empty($workspaceArray['modified'])) ? date('d M Y', strtotime($workspaceArray['modified'])) : 'N/A'; ?></span>
			</span>
	...
	<a class="btn <?php echo $class_name ?> tipText" title="<?php tipText('Update Workspace Details', false ); ?>"  href="<?php echo Router::Url(array('controller' => 'workspaces', 'action' => 'update_workspace', $project_detail['Project']['id'], $workspaceArray['id'], 'admin' => TRUE ), TRUE); ?>" id="btn_select_workspace" >
