<?php //
   ...
   $projectRole = projectRole($project_id, $current_user_id);
   $projectPermit = $owner = ($projectRole == 'Creator' || $projectRole == 'Owner' || $projectRole == 'Group Owner') ? true : false;

?>

<?php if(isset($project_teams) && !empty($project_teams)){ ?>
   <?php foreach ($project_teams as $key => $value) {
            $edata = $value['ed'];
            $costs = $value['cd'];
   ...
   <?php
      ...
      // COMPETENCY
      $project_skills = $psk['project_skills'];
      $project_subjects = $psb['project_subjects'];
      $project_domains = $psd['project_domains'];
   ...
   ?>
      <div class="ps-data-row" data-user="26">
         <div class="ps-col tm-col-1 team-col-info">
            ...
            <div class="team-user-status">
               <?php
                  ...
                  <span><a href="<?php echo Router::Url( array( 'controller' => 'resources', 'action' => 'people', 'user' => $edata['user_id'], 'project' => $project_id, 'tab' => 'tab_engagements', 'admin' => FALSE ), TRUE ); ?>" class="grp-title" title="Team Member Is Currently Absent<br>Click To Go To Engagements"><i class="absenceblack18"></i></a></span>
               <?php } ?>
            </div>
         </div>
         ...
         <div class="ps-col tm-col-5">
            <div class="progress-col-cont">
               <div class="compet-proj-bar-col">
                  <div class="schedule-bar" data-original-title="" title="">
                     <span class="blue bar-border ctip" title="" style="width:<?php echo $skill_percent; ?>%" data-original-title="Team Member has <?php echo $user_skills; ?> of <?php echo $project_skills; ?> Project Skills (<?php echo $skill_percent; ?>%)"></span>
                  </div>
               ...
               <div class="compet-proj-bar-col">
                  <div class="schedule-bar" data-original-title="" title="">
                     <span class="red2   bar-border ctip" title="" style="width: <?php echo $subject_percent; ?>%" data-original-title="Team Member has <?php echo $user_subjects; ?> of <?php echo $project_subjects; ?> Project Subjects (<?php echo $subject_percent; ?>%)"></span>
                  </div>
               ...
               <div class="compet-proj-bar-col">
                  <div class="schedule-bar" data-original-title="" title="">
                     <span class="green-bg   bar-border ctip" title="" style="width:<?php echo $domain_percent; ?>%" data-original-title="Team Member has <?php echo $user_domains; ?> of <?php echo $project_domains; ?> Project Domains (<?php echo $domain_percent; ?>%)"></span>
                  </div>
         ...
         <div class="ps-col tm-col-7">
            ...
            <a href="#" class="tipText" title="Tag Team Member" data-toggle="modal" data-target="#modal_nudge" data-remote="<?php echo Router::Url( array( 'controller' => 'tags', 'action' => 'add_tags_team_members', 'project' => $project_id, 'type' => 'project', 'selected' => $edata['user_id'], 'admin' => FALSE ), TRUE ); ?>"><i class="tagblack"></i></a>

            <a href="#" class="tipText" title="Nudge Team Member" data-toggle="modal" data-target="#modal_nudge" data-remote="<?php echo Router::url(array('controller' => 'boards', 'action' => 'send_nudge_board', 'project' => $project_id, 'type' => 'project_team', 'selected' => $edata['user_id'], 'admin' => false)); ?>"><i class="nudgeblack"></i></a>

            <?php if($projectPermit && ($edata['user_id'] != $current_user_id) ){
...
<script type="text/javascript">
   $(()=>{
         ...
         $('.user-profile-pic,.grp-title, .ctip').tooltip({
            placement: 'top',
            container: 'body',
            html: true,
