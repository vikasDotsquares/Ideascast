...
if(isset($wsp_activities_tasks) && !empty($wsp_activities_tasks)) {
	...
	foreach ($wsp_activities_tasks as $key => $tasks) {
		...
		$risk_counts = (isset($tasks['risk_counts']['risk_count']) && !empty($tasks['risk_counts']['risk_count'])) ? $tasks['risk_counts']['risk_count'] : 0;
...
?>
		...
		<div class="ps-data-row" data-id="<?php echo $element_id; ?>">
						...
						<div class="assign_div">
							<span class="style-popple-icon-out ">
								<a class="style-popple-icon" data-remote="<?php echo Router::url(['controller' => 'shares', 'action' => 'show_profile', $assign_user_id, 'admin' => FALSE], TRUE) ?>" id="trigger_edit_profile" data-target="#popup_modal" data-toggle="modal" data-original-title="" title="">
									<img src="<?php echo $user_image; ?>" class="user-image tipText" title="<?php echo $assign_user; ?>" data-content="<div><p><?php echo $assign_user; ?></p><p><?php echo $job_title; ?></p><p><?php echo $html; ?></p></div>" align="left" width="40" height="40" title="<?php echo $assign_user; ?>">
	...
	<?php } ?>
<?php }else{ ?>
	<div class="no-summary-found">No Tasks</div>
<?php } ?>
