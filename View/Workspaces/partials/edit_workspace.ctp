...
<!-- POPUP MODEL BOX CONTENT HEADER -->
	<div class="modal-header comm-head">
		...
		<h4 class="modal-title" id="myModalLabel">Edit Workspace</h4>
	...
	<div class="modal-body">
		<?php
			echo $this->Form->create('Workspace', array('url' => array('controller' => 'workspaces', 'action' => 'edit_workspace', $project_id, $workspace_id), 'class' => 'form-bordered', 'id' => 'modelFormAddWorkspace', 'data-async' => ""));
		?>
		<span id="date-error-message" style="" class="error-message text-danger"> </span>
			...
			<?php
				...
				echo $this->Form->input('ProjectWorkspace.workspace_id', [ 'type' => 'hidden', 'value' => $workspace_id ] );
				echo $this->Form->input('Workspace.id', [ 'type' => 'hidden', 'value' => $workspace_id ] );
			?>

			<?php
			$temlate_id = isset($this->request->data['Workspace']['template_id']) ? $this->request->data['Workspace']['template_id'] : 0;
			echo $this->Form->input('ProjectWorkspace.project_id', [ 'type' => 'hidden',  'value' => $project_id ] );
