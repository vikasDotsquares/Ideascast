...
if(isset($interest_users) && !empty($interest_users) && is_array($interest_users[0]) ){	
	
	if( isset($users) && !empty($users) ){		
		$finalArr = array_unique(array_merge($interest_users[0], $users));
	} else {
		$finalArr = $interest_users;
	}
} else if(isset($interest_users) && !empty($interest_users) && is_array($interest_users) ){
	if( isset($users) && !empty($users) ){		
    ...
    foreach ($finalArr as $user) {
        ...
        if(isset($user_data) && !empty($user_data)){
            ...
            if( $user[0] != $currentuser ) {
                    $btn_html = CHATHTML($user[0], $sketchdata['ProjectSketch']['project_id']);
            }
        }
