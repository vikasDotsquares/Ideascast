...
$section->addListItem(htmlspecialchars(strtoupper('Summary')), 0, 'myOwnStyle', $predefinedMultilevel, 'P-Style');
            ...
            if ((isset($workspace['id']) && !empty($workspace['id'])) && (isset($workspace['title']) && !empty($workspace['title']))) {
                ...
                if (isset($workspace['Area']) && !empty($workspace['Area'])) {
                    foreach ($workspace['Area'] as $area) {
                        $section->addListItem(strtoupper(htmlspecialchars(remove($workspace['title']))), 2, 'myOwnStyle', $predefinedMultilevel, 'P-Style');
