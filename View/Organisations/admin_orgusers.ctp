		...
		<div class="row">
			<div class="col-xs-12">
				...
				<ol class="breadcrumb">
					...
					<li><a href="<?php echo SITEURL; ?>sitepanel/organisations">Organisations</a></li>
					<li class="active">Users</li>
				</ol>
				<?php $organisationsName = $this->Common->userFullname($this->request->params['pass']['0']); ?>
			    <h3><?php echo $organisationsName; ?>: Users (<?php echo $count; ?>)</h3>
				</div>
					...
					<div class="box-header">
						<h3 class="box-title">
							<small>Per Page Records</small> </h3>
								<div class="margintop">
									<div class="col-lg-1 ">
										<?php echo $this->Form->create('User', array( 'url' => array('controller'=>'organisations', 'action'=>'orgusers',$this->request->params['pass']['0']), 'type' => 'file', 'class' => 'form-horizontal form-bordered', 'id' => 'per_page_show_form' )); ?>
										<?php 
						...
						<!--	<a class="btn btn-primary searchbtn" data-toggle="collapse" href="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
							...
							<!--<a class="btn btn-primary" href="<?php echo SITEURL ?>sitepanel/organisations/">Back</a></h3>-->
							<!--<a class="btn btn-primary" href="<?php echo SITEURL; ?>sitepanel/users/add"> Add User</a> -->
							
						</div>
						
						<div class="<?php echo $class; ?> search" id="collapseExample">
							<div class="well">
								<?php echo $this->Form->create('User', array( 'url' => array('controller'=>'organisations', 'action'=>'orgusers',$this->request->params['pass']['0']), 'type' => 'POST', 'class' => 'form-horizontal form-bordered', 'id' => 'search_page_show_form')); ?>
									<div class="modal-body">
										<div class="form-group">
											...
											<div class="col-lg-3">
												<?php echo $this->Form->input('User.emails', array('placeholder' => 'Enter email here...','type' => 'text','label' => false, 'value'=>$email,'div' => false, 'class' => 'form-control')); ?>
											</div>
											...
											<div class="col-lg-3">
													 ...
													 <?php echo $this->Form->input('UserDetail.country_id', array('options' => $this->Common->getCountryList(),  'empty' => 'Select Country', 'label' => false,'selected'=>$country,  'div' => false,  'onchange' => 'selectCity(this.options[this.selectedIndex].value)','class' => 'form-control')); ?>								  				  
											...
											<div class="col-lg-12" style="text-align:right; margin:20px 0 0 0">
												...
												<a class="btn btn-primary searchbtn" href="<?php echo SITEURL; ?>sitepanel/organisations/org_user_resetfilter/<?php echo$this->request->params['pass']['0']; ?>" >Close</a>
											</div>												
										</div>
									</div>
									</form>
					...
					<div class="box-body table-responsive">
						<table id="example2" class="table table-bordered table-hover">
							<thead>
								<tr>
									<th>#</th>
									<th><?php echo $this->Paginator->sort('User.UserDetail.first_name',__("Name"));?></th>
									...
									<td>
										<?php
											...
											if ($user['User']['status'] == 1) { ?>
												<button data-toggle="modal" data-target="#StatusBox" rel="deactivate" id="<?php echo $clasificationId; ?>"  class="btn btn-default RecordOrgUserUpdateClass"><i class="fa fa-fw fa-check alert-success"></i></button	>
										<?php	} else {
											 ?>
												<button data-toggle="modal" data-target="#StatusBox" rel="activate" id="<?php echo $clasificationId; ?>"  class="btn btn-default RecordOrgUserUpdateClass"><i class="fa fa-fw fa-times alert-danger"></i></button>
										<?php	}
										?>
									</td>
									<td><?php echo dateFormat($user['User']['created']); ?></td>
									<td>
										<?php 
										$viewURL = SITEURL."sitepanel/organisations/view/".$user['User']['id'];
										?>
										<a data-toggle="modal" class="viewuser"  data-target="#Recordview"   title="View User" data-whatever="<?php echo $viewURL; ?>"  data-tooltip="tooltip" data-placement="top" ><i class="fa fa-fw fa-eye"></i></a>
										
										<?php 
										$editURL = SITEURL."sitepanel/organisations/org_user_edit/".$user['User']['id']."/".$this->request->params['pass']['0']; 
										?>
										<a  class="editusers" data-target="#Recordeditss" href= "<?php echo $editURL; ?>"  title="Edit User" ><i class="fa fa-fw fa-edit"></i></a> 
										
										<?php 
										$deleteURL = SITEURL."sitepanel/organisations/org_user_delete/".$user['User']['id']; 
										?>
										
										<a data-toggle="modal" class="RecordDeleteClass" data-target="#deleteBox" rel="<?php echo $user['User']['id']; ?>"   title="Delete User" url="<?php echo $deleteURL; ?>"  data-tooltip="tooltip" data-placement="top" ><i class="fa fa-fw fa-trash-o"></i></a>
									</td>
								</tr>								
								<?php
										$num++;
...
$(document).on('click', '.RecordDeleteClass', function(){
	...
	$('#RecordDeleteForm').attr('action', '<?php echo SITEURL; ?>sitepanel/users/delete');
});


// Status click Update
$(document).on('click', '.RecordOrgUserUpdateClass', function(){
	id = $(this).attr('id');
	rel = $(this).attr('rel');
	$('#RecordStatusFormId').attr('action', '<?php echo SITEURL; ?>sitepanel/organisations/org_user_updatestatus');
	$('#recordID').val(id);
	if(rel == 'activate'){
		$('#recordStatus').val(1);
	}else{
