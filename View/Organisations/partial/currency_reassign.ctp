<?php
$current_list = $this->Scratch->currency_list($id);
$lists = $this->Scratch->currency_list($id, 1, ['where' => 'status = 1']); //
// pr($lists);
?>
	...
	<div class="modal-body clearfix">
		<input type="hidden" name="delete_id" id="delete_id" value="<?php echo $current_list[0]['cur']['id']; ?>">
		<div class="form-group row">
			<div class="col-sm-12"><label>Reassign Projects with Currency:</label></div>
			<div class="col-sm-12"><?php echo htmlentities($current_list[0]['cur']['title'], ENT_QUOTES, "UTF-8"); ?> (<?php echo $current_list[0]['cur']['sign']; ?>)</div>
		</div>
		<div class="form-group mb5">
			...
			if(isset($lists) && !empty($lists)){
				...
				<option value="<?php echo $list['cur']['id']; ?>"><?php echo $list['cur']['title']; ?> (<?php echo $list['cur']['sign']; ?>)</option>
				<?php } ?>
				<?php } ?>
			</select>
		</div>
...
<script type="text/javascript">
	$(() => {
		...
		$(".submit-assign").off('click').on('click', function(event) {
			...
			if(update_id != '' && update_id !== undefined){
				$.ajax({
		    		url: $js_config.base_url + 'organisations/currency_reassign',
		    		type: 'POST',
		    		dataType: 'json',
