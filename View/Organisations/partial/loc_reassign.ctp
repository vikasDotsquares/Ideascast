<?php
$current_list = $this->Scratch->loc_type_list($id);
$lists = $this->Scratch->loc_type_list($id, 1, ['where' => 'status = 1']); //
// pr($lists);
?>
	...
	<div class="modal-body clearfix">
		...
		<div class="form-group row">
			<div class="col-sm-12"><label>Reassign Locations with Type:</label></div>
...
<script type="text/javascript">
	$(() => {
		...
		$(".submit-assign").off('click').on('click', function(event) {
			...
			if(update_id != '' && update_id !== undefined){
				$.ajax({
		    		url: $js_config.base_url + 'organisations/loc_reassign',
		    		type: 'POST',
		    		dataType: 'json',
