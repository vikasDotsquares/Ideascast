<?php
$current_list = $this->Scratch->program_type_list($id);
$lists = $this->Scratch->program_type_list($id, 1);
...
?>
	...
	<div class="modal-body clearfix">
		<input type="hidden" name="delete_id" id="delete_id" value="<?php echo $current_list[0]['types']['id']; ?>">
		<div class="form-group row">
			<div class="col-sm-12"><label>Reassign Programs with Type:</label></div>
			<div class="col-sm-12"><?php echo htmlentities($current_list[0]['types']['type'], ENT_QUOTES, "UTF-8"); ?></div>
		</div>
		<div class="form-group mb5">
			...
			if(isset($lists) && !empty($lists)){
				...
				<option value="<?php echo $list['types']['id']; ?>"><?php echo $list['types']['type'];//.' ('.$prj_count.')'; ?></option>
				<?php } ?>
				<?php } ?>
			</select>
		</div>
...
<script type="text/javascript">
	$(() => {
		...
		$(".submit-assign").off('click').on('click', function(event) {
			...
			if(update_id != '' && update_id !== undefined){
				$.ajax({
		    		url: $js_config.base_url + 'organisations/program_type_reassign',
		    		type: 'POST',
		    		dataType: 'json',
