<?php
if(isset($loc_lists) && !empty($loc_lists)){
	// pr($org_lists);
	$list_count = count($loc_lists);
	foreach ($loc_lists as $key => $list) {
...
<div class="task-type-list-data-row" data-id="<?php echo $list['ot']['id']; ?>" data-title="<?php echo htmlentities($list['ot']['title'], ENT_QUOTES, "UTF-8"); ?>">
	...
	<div class="task-type-list-col type-list-col-3 task-type-list-actions">
		...
		<?php if($counter > 0){ ?>
		<a class="tipText reassign-item" href="#" title="Reassign" data-toggle="modal" data-target="#model_reassign" data-remote="<?php echo Router::url(['controller' => 'organisations', 'action' => 'loc_reassign', $list['ot']['id'], 'admin' => FALSE], TRUE); ?>"> <i class="reassignblackicon"></i></a>
...
<script type="text/javascript">
	$(() => {
		$('.list-counter', $('#tab_loc_type')).text(" (" + $('.task-type-list-data-row', $('#tab_loc_type')).length + ")");
	})
</script>