	...
	<?php echo $this->Session->flash(); ?>
	 <div class="box-header" style="cursor: move;">
			<ol class="breadcrumb">
				...
				<li ><a href="<?php echo SITEURL ?>sitepanel/organisations" data-original-title="" title=""><?php echo $organisationname;?></a></li>
				<li class="active"> Edit Linked Domain</li>
			</ol>
	  </div>
	  <div class="panel-heading">
         <h3 class="panel-title">
            Edit Linked Domain <?php //echo $organisationname;?>
		...
		<div class="panel-body form-horizontal">
			...
			<?php echo $this->Form->create('OrgSetting', array( 'url' => array('controller'=>'organisations', 'action'=>'edit_domain'), 'type' => 'file', 'class' => 'form-horizontal form-bordered', 'enctype' => 'multipart/form-data', 'id' => 'domainAdd')); ?>
...
* Domain should start and end with characters only.<br/>';
								echo $this->Form->input('OrgSetting.subdomain', array('type' => 'text','label' => false, 'div' => false, 'class' => 'form-control', 'autocomplete'=>'off', 'disabled'=>'disabled', 'placeholder'=>'Linked Domain name','minlength'=>'2', 'maxlength'=>'32')); ?>
				...
				<?php
				$sd = $ed = '';
				if(isset($this->request->data['OrgSetting']['start_date'])){
					$sd = $this->request->data['OrgSetting']['start_date'];
					$this->request->data['OrgSetting']['start_date'] = date('d/m/Y',strtotime($this->request->data['OrgSetting']['start_date']));

				}
				if(isset($this->request->data['OrgSetting']['end_date'])){
					$ed = $this->request->data['OrgSetting']['end_date'];
					$this->request->data['OrgSetting']['end_date'] = date('d/m/Y',strtotime($this->request->data['OrgSetting']['end_date']));
				}
						...
						<div class="col-md-6">
							<div class="form-group">
							  ...
							  <div class="col-lg-8">
									...
									echo $this->Form->input('OrgSetting.start_date', array('type' => 'text','label' => false, 'div' => false, 'class' => 'form-control dates input-small hasDatepicker', 'autocomplete'=>'off', 'id'=>'start_date','placeholder'=>'Start Date')); ?>
									...
									echo $this->Form->input('OrgSetting.id', array('type' => 'hidden','label' => false, 'div' => false));
									?>
				...
				<div class="modal-footer clearfix">
					...
					<button type="submit" class="btn btn-success"><!--<i class="fa fa-fw fa-check"></i>--> Update</button>
					<a class="btn btn-danger" href="<?php echo SITEURL ?>sitepanel/organisations/list_domain/<?php echo $this->request->data['OrgSetting']['user_id']; ?>">Close</a>
...
<script type="text/javascript" >
	...
	var sd = '<?php echo date('Y-m-d',strtotime($sd)); ?>';
	var sdU = '<?php echo date('d/m/Y', strtotime($sd)); ?>';
	var sdate = '<?php echo date('Y-m-d'); ?>';
	var sdateU = '<?php echo date('d/m/Y'); ?>';
	...
	//alert(setD);
		...
		ToEndDate.setDate(ToEndDate.getDate()+365);
			...
			weekStart: 1,
			format: 'dd/mm/yyyy',
			 // startDate: '<?php echo $this->request->data['OrgSetting']['start_date']; ?>',
			  startDate: setD,
			autoclose: true
			...
			startDate = new Date(selected.date.valueOf());
			startDate.setDate(startDate.getDate(new Date(selected.date.valueOf())));
			$('#end_date').datepicker('setStartDate', startDate).val($('#start_date').val());
			...
			$('#end_date').datepicker({
				weekStart: 1,
				format: 'dd/mm/yyyy',
				startDate: '<?php echo date('d/m/Y', strtotime($sd)); ?>',
			//	endDate: '<?php echo $this->request->data['OrgSetting']['end_date']; ?>',
				autoclose: true
			}).on('changeDate', function(selected){
