<?php
if(isset($allbloglist) && !empty($allbloglist) && count($allbloglist) > 0){

	foreach($allbloglist as $listView){
...
?>
	<div class="accordion-group" id="bloglistview<?php echo $listView['Blog']['id'];?>" >
		...
		<a  class="blog_data_admin" id="target<?php echo $listView['Blog']['id'];?>"  data-project="<?php echo $listView['Blog']['project_id']; ?>" data-rel="<?php echo $listView['Blog']['id'];?>"  data-id="#collapse<?php echo $listView['Blog']['id'];?>">
			<em class="icon-fixed-width fa fa-plus-square-o"></em>
			<img src="<?php echo SITEURL.'img/blog-icon-black-300.png' ?>" width="22">
			<span style="font-weight:600;"><?php echo $listView['Blog']['title'];?></span>
		 </a>
		...
		<div class="blog-full-details" style="margin: 0 0 0 48px;">
			<div class="blog_details">
				...
				$endTimeStamp = strtotime(date('Y-m-d g:i:s'));
				...
				?>&nbsp;&nbsp;
					...
					<a id="admin_blog_like" data-value="<?php echo $listView['Blog']['id'];?>" style="cursor:pointer;" class="btn btn-xs btn-default ">
				...
				<?php  }  ?>
					...
					if( $listView['Blog']['user_id'] == $this->Session->read('Auth.User.id') || $deletepermission == true ) {
				?>
					<a id="confirm_blog_delete" class="btn btn-xs btn-danger tipText delete_blog" data-original-title="Delete" data-value="<?php echo $listView['Blog']['id'];?>"><i class="fa fa-trash"></i></a>
