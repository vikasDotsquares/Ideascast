...
<!-- POPUP MODEL BOX CONTENT HEADER -->
	...
	<?php if(isset($this->data) && !empty($this->data)) { ?>
		<span class="del_confirmation">
	        <a href="" class="btn btn-xs btn-success del_success tipText" data-projectid="<?php echo element_project($el['id']);?>" data-id="<?php echo ($this->data['Reminder']['id']); ?>" title="Confirm"><i class="fa fa-check"></i></a>
				...
				$.ajax({
					...
					success: function( response, status, jxhr ) {
						...
						if( response.success ) {
							...
							$.save_clicked = false;
							$(".line-header[data-target='#project_row_" + project_id +"']", window.parent.document).find(".tasktype-clear").trigger('click');
