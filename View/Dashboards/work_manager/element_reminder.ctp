<?php 
// echo $this->Html->css('projects/datetime/jquery-ui');
...
<!-- POPUP MODEL BOX CONTENT HEADER -->
	...
	<div class="modal-body elements-list">
		...
		<div class="form-group">
			...
			<div class="title-group"> 
				<?php 
				$remind_date = (isset($this->data['Reminder']['reminder_date']) && !empty($this->data['Reminder']['reminder_date'])) ? date('d-m-y: gA',strtotime($this->data['Reminder']['reminder_date'])) : '';
	...
	<?php if(isset($this->data) && !empty($this->data)) { ?>
		<span class="del_confirmation">
	        <a href="" class="btn btn-xs btn-success del_success tipText" data-id="<?php echo ($this->data['Reminder']['id']); ?>" title="Confirm"><i class="fa fa-check"></i></a>
	        <a href="" class="btn btn-xs btn-danger del_cancel tipText" title="Cancel"><i class="fa fa-times"></i></a>
	    </span>
		 <button type="button" data-id="<?php echo ($this->data['Reminder']['id']); ?>" class="btn btn-danger trash_task_reminder"><i class="fa fa-trash"></i> Delete</button>
	<?php } ?>
		 <button type="button"  class="btn btn-success submit_reminder">Save</button>
	...
	//	var start = '<?php // echo date("d-m-Y", strtotime($el['start_date'])); ?>';
		...
		var reminder_date = $('#reminder_date').datetimepicker({
			...
			showTimezone:false,
			ampm: true,
			hour: <?php echo date('H')+1; ?>,
			hourMin: <?php echo date('H')+1; ?>,
			hourMax: 23,
