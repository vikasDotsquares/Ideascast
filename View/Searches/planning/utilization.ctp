<div class="utilization-list-data-inner">
<?php
$result = [];
if(isset($data) && !empty($data)){
    foreach ($data as $key => $value) {
        $dates = array_chunk($value[0], 6, true);
        // $dates['user_id'] = $value['util']['user_id'];
...
?>
    ...
    $dates_array = [];
    ?>
    <?php if(isset($date_type) && $date_type == 'daily' ){ ?>
    <div class="abs">
        <?php
            foreach ($result[0]['dates'] as $values) {
                $dt = array_shift($values);
                $day_str = date('l', strtotime($dt));
                $day = date('N', strtotime($dt));
                $bg_class = '';
                if($day == 6 || $day == 7){
                    $bg_class = 'col-weekend';
                }
        ?>
                <div class="util-col <?php echo $bg_class; ?> tipText" title="<?php echo $day_str; ?>" style="visibility: hidden;">
                    <?php echo ($date_type == 'monthly') ? date('M, Y', strtotime($dt)) : date('d M, Y', strtotime($dt)); ?>
                </div>
            <?php } ?>
    </div>
    <?php } ?>
    <div class="util-1">
        <div class="utilization-list-header">
            <div class="util-col">
                People <span class="utill-total">(0)</span>
            </div>
        </div>
        <div class="utilization-left-cont-outer">
        <?php
        ...
        <div class="utilization-list-header">
            <?php
                foreach ($result[0]['dates'] as $values) {
                    $dt = array_shift($values);
                    // pr($dt);
                    $day_str = ($date_type == 'daily') ? date('l', strtotime($dt)) : ($date_type == 'weekly' ? 'Week Commencing' : 'Month');
                    $day = date('N', strtotime($dt));
                    $bg_class = '';
                    if(($day == 6 || $day == 7)  && $date_type == 'daily'){
                        $bg_class = 'col-weekend';
                    }
            ?>
                    <div class="util-col <?php echo $bg_class; ?> tipText" title="<?php echo $day_str; ?>">
                        <?php echo ($date_type == 'monthly') ? date('M, Y', strtotime($dt)) : date('d M, Y', strtotime($dt)); ?>
                    </div>
                <?php } ?>
        </div>
        <div class="util-right-sec">
    ...
    <?php }else{ ?>
        <div class="no-utilization">no utilization</div>
    <?php } ?>
    </div>
<?php if(isset($data) && !empty($data)){ ?>
<script type="text/javascript">
    $(()=>{
        ...
        // SET SAME AMOUNT OF SCROLL OF HEADER AND DATES LIST SECTION
        /*$('.util-right-sec').on('scroll', function(event) {
