...
<!-- POPUP MODEL BOX CONTENT HEADER -->
	...
	<div class="modal-body">
		<?php
			echo $this->Form->create('Workspace', array('url' => array('controller' => 'entities', 'action' => 'task_list_save_workspace', $response['project_id']), 'class' => 'form-bordered', 'id' => 'modelFormUpdateWorkspace', 'data-async' => "")); 
		?>
		...
		<span id="date-error-message" style="" class="error-message text-danger"> </span>
			...
			<div class="form-group clearfix">
				...
				<div class="wsel_title wsp_<?php echo workspace_status($this->data['Workspace']['id']); ?>"  >
		...
		<div class="date_constraints_wrappers" style="">
			<div class="form-group clearfix col-sm-6">
				...
				<div class="input-group">
					<?php 
					if( isset($this->data['Workspace']['start_date']) && !empty($this->data['Workspace']['start_date']) ){
						$wspStartDate = date('d M Y', strtotime($this->data['Workspace']['start_date']));
					} else {
						$wspStartDate = '';
					}	
					echo $this->Form->input('Workspace.start_date', [ 'type' => 'text', 'label' => false, 'div' => false, 'id' => 'start_date', 'required' => false,  'readonly' => 'readonly', 'class' => 'form-control dates input-small', 'value'=>$wspStartDate ]); 
			...
			<div class="form-group clearfix col-sm-6">
				...
				<div class="input-group">
					<?php 
					if( isset($this->data['Workspace']['end_date']) && !empty($this->data['Workspace']['end_date']) ){
						$wspEndDate = date('d M Y', strtotime($this->data['Workspace']['end_date']));
					} else {
						$wspEndDate = '';
					}
	...
	$mindate =  isset($date['start_date']) && !empty($date['start_date']) ? date("d M Y",strtotime($date['start_date'])) : date("d M Y");
