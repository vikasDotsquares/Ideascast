...
<div class="clearfix col-sm-12 inner-first cost-label-sec nopadding">
    <div class="box-header filters" style="padding: 10px;">
        <div class="org-chart-select peoplechart">
            <label class="custom-dropdown">
                <select class="aqua" name="people_from" id="people_from" style=" width: 100%; margin-bottom: 0;">
        ...
        <div class="org-chart-select popup-select-icon peoplechart">
            <select class="aqua" name="specific_items" id="specific_items" disabled="" multiple="" style=" width: 100%; margin-bottom: 0;"></select>
        </div>
        <div class="org-chart-select">
            <label class="custom-dropdown">
                <select class="aqua" name="dotted_lines" id="dotted_lines" style=" width: 100%; margin-bottom: 0;">
                    <option value="direct">Direct Lines</option>
                    <option value="direct_dotted">Direct and Dotted Lines</option>
                </select>
            </label>
        </div>
        <button class="btn btn-success btn-org-chart oc-disable" type="button" >Show</button>
    </div>
</div>
<div class="border1 org-chart-wrapper" id="oc_analyticsIframe" style="width: 100%; height: 100%; overflow: hidden;">
    <div class="col-sm-12 partial_data box-borders " style="padding: 10px;">
        <div class="col-sm-12 box-borders select-project-sec">
            <div class="no-data">SELECT PEOPLE</div>
        </div>
    </div>
</div>

<div id="oc_menuPanel" class="oc_menuGroup">
    <select id="oc_peopleList" class="oc_peopleDropDownList" onchange="oc_expandUser(this.value);">
        <option value="">Select People</option>
    </select>
    <button id="oc_expandAll" class="oc_expandButton" onclick="oc_expandAll();">Expand All</button>
    <button id="oc_collapseAll" class="oc_collapseButton" onclick="oc_collapseAll();">Collapse All</button>
</div>
...
<script type="text/javascript">
    $(() => {
        ...
        $.btn_status = function(data){
            var people_from = $('#people_from').val();
            var specific_items = $('#specific_items').val() || [];
            var dotted_lines = $('#dotted_lines').val();

            $('.btn-org-chart').addClass('oc-disable');


            if(people_from != '' && dotted_lines != '' ) {
                var pfval = (people_from == 'community' || people_from == 'all_projects' || people_from == 'created_projects' || people_from == 'owner_projects' || people_from == 'shared_projects') ? true : false;
                if( !pfval && specific_items.length > 0 ){
                    $('.btn-org-chart').removeClass('oc-disable');
                }
                else if(pfval){
                    $("#specific_items").prop('disabled', false).multiselect('dataprovider', []);
                    $('.btn-org-chart').removeClass('oc-disable');
                }
            }
        }

        $.get_specific_item = function(data){
            $('#specific_items').empty();
            $("#specific_items").prop('disabled', false).multiselect('dataprovider', []);
            $.ajax({
                ...
                success: function(response){
                    if(response.success){
                        if(response.content){
                            ...
                            $("#specific_items").prop('disabled', false).multiselect('dataprovider', content);
                            // $("#specific_items").val([$js_config.project_id]).multiselect('select', [$js_config.project_id]);
                            $.btn_status();
                        }
                    }
                }
        ...
        $('#people_from').on('change', function(event) {
            event.preventDefault();
            var type = $(this).val();
            $.btn_status();
            if(type == '' || type == undefined) {
                $("#specific_items").prop('disabled', true).multiselect('dataprovider', []);
            }
            else {
                var data = {type: type };
                $.get_specific_item(data);
            }
        })
        ...
        ;($.resizeWrapper = function(){
            setTimeout(() => {
                $('#oc_analyticsIframe').animate({
                    minHeight: (($(window).height() - $('#oc_analyticsIframe').offset().top) ) - 12,
                    height: (($(window).height() - $('#oc_analyticsIframe').offset().top) ) - 12,
                }, 1)
            }, 200);
        })();
        $specific_items = $('#specific_items').multiselect({
...
<script type="text/javascript">
    const oc_base_path = $js_config.base_url;
    const oc_image_path = oc_base_path + "images/icons/org_chart/";
    const oc_user_path = '';//oc_base_path + "uploads/user_images/";

    var oc_chart, allNodes;
    var oc_format = d3.format(",d");

    $('.btn-org-chart').on('click', function(event) {
        var people_from = $('#people_from').val();
        var specific_items = $('#specific_items').val() || [];
        var dotted_lines = $('#dotted_lines').val();
        /*if($(this).is(':visible')) {
            $('#oc_analyticsIframe').html('<div class="col-sm-12 partial_data box-borders " style="padding: 10px;">\
                            <div class="col-sm-12 box-borders select-project-sec">\
                                <div class="no-data">SELECT PEOPLE</div>\
                            </div>\
                        </div>');
        ...
        $('.oc_menuGroup').show();
        $('#oc_analyticsIframe').html('');
        //get data
        d3.json($js_config.base_url + "subdomains/org_chart_json/", {
                    method:"POST",
                    body: JSON.stringify({
            ...
            ).then(function (oc_srcdata) {

            oc_data = oc_srcdata.map(d=>{
                const width = 400; //Math.round(Math.random()*50+300);
                const height = 160; //Math.round(Math.random()*20+130);
                const cornerShape = 'CIRCLE'; //['ORIGINAL','ROUNDED','CIRCLE'][Math.round(Math.random()*2)];
                const nodeImageWidth = 110;
                const nodeImageHeight = 110;
                const centerTopDistance = 72;
                const centerLeftDistance = 70;
                const expanded = false;

                const  titleMarginLeft = 146; //nodeImageWidth/2+20+centerLeftDistance
                const  contentMarginLeft = 171;//width/2+25
                return {
                    nodeId:d.id,    // node identificator
                    code:d.code,    // node type
                    parentNodeId:d.parentId,  // parent node identificator
                    width:width,   // node card width
                    height:d.code=='TR' ? 143 : height,  // node card height
                    borderWidth:2,  // node card border width
                    borderRadius:10,  // node card border radius
                    ...
                    nodeImage:{        // node image properties
                        url:(d.code=='TR' ? oc_image_path : oc_user_path) + d.imageUrl,    // url to image
                    ...
                    nodeIcon: {
                    icon: oc_base_path + 'images/icons/communityglyphGrey18x18.png',
                    size: 48
                    },
                    notInYourOrganization: d.notInYourOrganization, //node glyph
                    // node card content
                    template:`<div>
                                <div style="margin-left:${titleMarginLeft}px;
                                            margin-top:` + (d.code=="TR" ? `24px` : `12px`) + `;
                                            font-family: 'Open Sans';
                                            ` + (d.code=="TR" ? `font-size:20px;` : `font-size:24px;`) + `
                                            color: #444;
                                            font-weight:bold;
                                            white-space: nowrap;
                                            overflow: hidden;
                                ...
                                <div style="margin-left:${contentMarginLeft}px;
                                    ...
                                    <div style="white-space: nowrap;
                                            ...
                                            text-overflow: ellipsis;
                                        ">
                                        <img src='${oc_image_path}Organization Green 48x48.png' width='18px' height='18px' style='padding-right:6px; position:absolute; left:${titleMarginLeft}px;'>
                                        ${d.organization}</div>
                                    <div style="white-space: nowrap;
                                            overflow: hidden;
                                            text-overflow: ellipsis;
                                        ">
                                        <img src='${oc_image_path}Location Green 48x48.png' width='18px' height='18px' style='padding-right:6px; position:absolute; left:${titleMarginLeft}px;'>
                                        ${d.location}</div>
                                    <div style="white-space: nowrap;
                                            overflow: hidden;
                                            text-overflow: ellipsis;
                                        ">
                                        <img src='${oc_image_path}Department Green 48x48.png' width='18px' height='18px' style='padding-right:6px; position:absolute; left:${titleMarginLeft}px;'>
                                        ${d.department}</div>
                                    </div>
                                </div>`),
                    ...
                    connectorLineWidth:d.code == 'DL' ? '3' : '4',      // Edge width
                    dashArray:d.code == 'DL' ? '10' : '',              // We can have stripped edges, you should specify dashArray for it (google: svg dashArray)
            ...
            //load people list
            //empty drop down
            document.getElementById("oc_peopleList").innerHTML = "";

            let oc_peopleNamesIDs = oc_srcdata.filter(d => d.code === "RT" || d.code === "IR").sort((a, b) => d3.ascending(a.name, b.name));

            let oc_peopleNameList = d3.map(oc_peopleNamesIDs, d => d.name).keys();
            let oc_peopleIDs = d3.map(oc_peopleNamesIDs, d => d.id).keys();

            //add all entry
            let oc_option = document.createElement("option");
            oc_option.text = "People (" + oc_format(oc_peopleNameList.length) + ")";
            oc_option.value = "All People";
            oc_peopleList.add(oc_option);

            //add entries
            for (let oc_i = 0; oc_i < oc_peopleNameList.length; oc_i++) {
                let oc_option = document.createElement("option");
                oc_option.text = oc_peopleNameList[oc_i];
                oc_option.value = oc_peopleIDs[oc_i];
                oc_peopleList.add(oc_option);
            };

            oc_chart = new OrgChart();

            oc_chart
                .container('#oc_analyticsIframe')
                .data(oc_data)
                .svgWidth(document.getElementById('oc_analyticsIframe').clientWidth)
                .svgHeight(document.getElementById('oc_analyticsIframe').clientHeight)
                .initialZoom(0.5)
                .onNodeClick(d=> oc_showUserProfile(d))
                .render();

            //handle resize of browser window
            window.addEventListener('resize', oc_windowResize);

        })
        .catch(function(oc_err) {
            //display NO MATCHING PEOPLE
            //throw error
            console.log(oc_err);
            $('#oc_analyticsIframe').html('<div class="col-sm-12 partial_data box-borders " style="padding: 10px;">\
                <div class="col-sm-12 box-borders select-project-sec">\
                    <div class="no-data">No Org Chart Data</div>\
                </div>\
            </div>');
            throw new Error("No Org Chart Data - Managed Exit.");
    ...
    class OrgChart {
        ...
        render() {
            ...
            layouts.treemap = d3.tree().size([calc.chartWidth, calc.chartHeight])
                ...
                .nodeSize([calc.nodeMaxWidth + 40, calc.nodeMaxHeight + attrs.depth]) //40 gap between nodes
            ...
            */
            ...
            attrs.root.children ? attrs.root.children.forEach(d => this.expandSomeNodes(d)) : '';
            ...
            const svg = container
                .patternify({
                    tag: 'svg',
                    selector: 'oc_svg-chart-container'
                })
        ...
        update({
            ...
            nodeEnter
                ...
                .attr('x',d => d.id=="0" ? -54 : d.width/2-10)
                //.attr('y',d=> d.height/2-10)
                .attr('y',d => d.id=="0" ? 44 : d.height/2-10)
                //.attr('text-anchor','end')
                .attr('text-anchor',d => d.id=="0" ? 'start' : 'end')
                .text(d=> d.data.totalSubordinates + (d.data.totalSubordinates == 1 ? ' Total Relation' : ' Total Relations'))
                .attr('fill','#777')
                .attr('font-weight','bold')
                .attr('font-size', d => d.id=="0" ? '16px' :'14px')
                .attr('display', d => d.data.totalSubordinates == 0 ? 'none' : '')
            ...
            nodeEnter
                ...
                .attr('x', d => d.id=="0" ? -54 : -d.width/2+10)
                //.attr('y',d=> d.height/2 - 10)
                .attr('y', d => d.id=="0" ? 24 : d.height/2 - 10)
                .text(d=> d.data.directSubordinates + (d.data.directSubordinates == 1 ? ' Direct Relation' : ' Direct Relations'))
                .attr('fill','#777')
                .attr('font-weight','bold')
                .attr('font-size', d => d.id=="0" ? '16px' :'14px')
                .attr('display', d => d.data.directSubordinates == 0 ? 'none' : '')
        ...
        // Add node icon image inside node
            nodeEnter
                ...
                .attr('x', d => -118) //d=>-d.width/2+5'')
                .attr('y', d => 2)//d.height/2 - d.data.nodeIcon.size-5)
                .attr('display', d => d.data.notInYourOrganization == 0 ? 'none' : '')
                .attr('class', 'tipText')
                .attr('title', 'Not In Your Organization')
                // .append('title').text('Not In Your Organization')
            ...
            nodeButtonGroups
                ...
                .append('title').text('Click to Expand/Collapse')
            ...
            // IR Node button circle group
            const nodeButtonGroupsIR = nodeEnter
                .patternify({
                    tag: 'g',
                    selector: 'node-button-g-ir',
                    data: d => [d]
                })
                .style('cursor', 'default')


            // IR Add button circle
            nodeButtonGroupsIR
                .patternify({
                    tag: 'circle',
                    selector: 'node-button-circle-ir',
                    data: d => [d]
                })
                .append('title').text('Indirect Report')
            ...
            nodeUpdate.select('.node-image-rect')
                .attr('fill', ({
                    id
                }) => `url(#` + ((id.indexOf('-') != -1) ?  id.substring(id.indexOf('-')+1) : id) + `)`)
                .attr('width', ({
                    imageWidth
                }) => imageWidth)
                .attr('height', ({
            ...
            const userSelection = document.getElementById("oc_peopleList").value;
            const dl_userSelection = '-' + userSelection;
            nodeUpdate.select('.node-rect')
                ...
                .style("fill", ({
                    id
                }) => id == '0' ? '#ebf1fb' : id == userSelection || id.substring(id.indexOf('-')) == dl_userSelection ? '#e7d685' : '#f1f3f4')
                .attr('filter', ({
                    dropShadowId
                }) => dropShadowId)
            ...
            // IR Move node button group to the desired position
            nodeUpdate.select('.node-button-g-ir')
                .attr('transform', d => {
                    return `translate(0,-${d.data.height/2+23})`
                })
                .attr('display', d => {
                    if (d.data.code=="IR") {
                        return '';
            ...
            // IR Restyle node button circle
            nodeUpdate.select('.node-button-circle-ir')
                .attr('r', 10)
                .attr('stroke-width', '4')
                .attr('fill', '#fff')
                .attr('stroke', '#777')
        ...
        */
        ...
        setExpanded(id, expandedFlag) {
            ...
            if (node) {
                node.data.expanded = expandedFlag;
            }
            ...
            const nodes = attrs.allNodes.filter(({
                data
            }) => data.nodeId.indexOf('-') != -1 && data.nodeId.substring(data.nodeId.indexOf('-')) == dl_id)
            if (nodes) {
                nodes.forEach( d => d.data.expanded = expandedFlag);
            }

            // First expand all nodes
            attrs.root.children ? attrs.root.children.forEach(d => this.expand(d)) : '';


            // Expand root's children in case they are collapsed
        ...
        // Function which collapses passed node and its descendants
        collapse(d) {
            if (d.children) {
                d._children = d.children;
        ...
        expandCollapseAll(flag) {
            ...
            // Loop over and recursively update collapsed children's descendants
            if (attrs.root._children) {
                attrs.root._children.forEach(d => {
                    this.setExpansionFlagToChildren(d, flag)
                })
            }
        ...
        // Function which expands passed node and its descendants
        expand(d) {
            if (d._children) {
                d.children = d._children;
    ...
    function oc_zoomToCenter () {

    const svg = d3.selectAll('.oc_svg-chart-container');
    const svgElement = d3.selectAll('.oc_chart');
    ...
    // Get zooming function
    behaviors.zoom = d3.zoom().on("zoom", d => oc_chart.zoomed(d));
    ...
    function oc_windowResize () {

        //resize svg
        d3.selectAll(".oc_svg-chart-container")
            .attr('width', document.getElementById('oc_analyticsIframe').clientWidth)
            .attr('height', document.getElementById('oc_analyticsIframe').clientHeight);

        //redraw
        const attrs = oc_chart.getChartState();
        oc_chart.update(attrs.root);

    }

    function oc_showUserProfile (oc_id) {

        if (oc_id != "0") { //not root node

            //if dotted line then remove parent id
            if (oc_id.indexOf('-') != -1) {oc_id = oc_id.substring(oc_id.indexOf('-')+1)}
                // console.log(oc_id)
            //open user profile here with oc_id
            $('#popup_modal').modal({
                remote: $js_config.base_url + 'shares/show_profile/' + oc_id
            })
            .modal('show');
    ...
    function oc_expandUser (oc_id) {

        document.getElementById("oc_peopleList").blur();

        if (oc_id != 'All People') {

            document.getElementById("oc_collapseAll").disabled = true;
            document.getElementById("oc_expandAll").disabled = true;
            document.getElementById("oc_peopleList").disabled = true;

            //expand user (report to and dotted lines)
            oc_chart.setExpanded(oc_id,true);

            document.getElementById("oc_collapseAll").disabled = false;
            document.getElementById("oc_expandAll").disabled = false;
            document.getElementById("oc_peopleList").disabled = false;

        }
    }

    function oc_expandAll() {

        document.getElementById("oc_expandAll").blur();
        document.getElementById("oc_collapseAll").disabled = true;
        document.getElementById("oc_expandAll").disabled = true;
        document.getElementById("oc_peopleList").disabled = true;

        //expand all
        oc_chart.expandCollapseAll(true);
        oc_zoomToCenter();
    ...
    function oc_collapseAll() {

        document.getElementById("oc_collapseAll").blur();
        document.getElementById("oc_collapseAll").disabled = true;
        document.getElementById("oc_expandAll").disabled = true;
        document.getElementById("oc_peopleList").disabled = true;
        document.getElementById("oc_peopleList").value = 'All People';


        //collapse all
        oc_chart.expandCollapseAll(false);
        oc_zoomToCenter();
