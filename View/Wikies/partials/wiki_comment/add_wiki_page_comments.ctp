...
<?php
echo $this->Form->create('WikiPageComment', array('url' => array('controller' => 'wikies', 'action' => 'wiki_page_comment_save'), 'class' => 'form-bordered',  'data-async' => "","id"=>"wikipagecommentform", 'enctype' => 'multipart/form-data'));
?>
<!-- POPUP MODEL BOX CONTENT HEADER -->
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    <h3 class="modal-title" id="createModelLabel">Add Wiki Comment</h3>
</div>
...
?>
    <div class="form-group">
        <label for="comments" >Description:</label>
            <?php echo $this->Form->textarea('WikiPageComment.description', [ 'type' => 'text', 'class' => 'form-control', 'required' => true, 'div' => false, 'id' => 'comments', 'escape' => true, 'placeholder' => 'max chars allowed 1000', 'rows' => 5, 'label' => false]); ?>
        ...
        <ul class="list-group" id="comment_uploads_list">
            ...
            if (isset($this->request->data["WikiPageCommentDocument"]) && !empty($this->request->data["WikiPageCommentDocument"])) {
                foreach ($this->request->data["WikiPageCommentDocument"] as $val_doc) {
                    echo '<li class="todoimg list-group-item"><a href="' . SITEURL . WIKI_PAGE_DOCUMENT . $val_doc['document_name'] . '" class="todoimglink tipText" title="' . $val_doc['file_name_original'] . '" download="download" >';
...
<div class="modal-footer">
    <button type="button"  class="btn btn-success submit_wiki_page_comments">Save</button>
