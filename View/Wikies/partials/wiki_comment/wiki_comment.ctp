...
<div class="col-sm-8 col-md-8 col-lg-9 wiki-right-section">
    <div id="comments_list" class="tast-list-left-main">
        ...
        $wikipagecomments_rating = $this->requestAction(array("action" => "get_wiki_page_comment_rating", $project_id, $user_id, $wiki_id));
        $commentparam = "wikies/add_wiki_page_comments/" . $project_id . "/" . $this->Session->read('Auth.User.id') . "/" . $wiki_id;
        //pr($wikipagecomments);
        ?>
...
//                                            $rfc_1123_date = date('Y-m-d h:i:s A', strtotime($wikipagecomment['WikiPageComment']['created']));                                             
...
//                                            echo date('Y-m-d h:i:s A', $sertimestamp);
                ...
                <div class="tab-pane fade teammembercomment" id="people">
                    <div class="row">
                    <div class="col-lg-4">
                    <?php
                    $users = $this->Wiki->get_all_wiki_users($wiki_id);
                    $remote = SITEURL . "wikies/get_wiki_page_comment_by_user/" . $project_id . "/" . $this->Session->read('Auth.User.id') . "/" . $wiki_id;
                    echo $this->Form->input("user_id", array("id" => "people_change", "multiple" => true, "type" => "select", "options" => $users, "div" => false, "label" => false, "class" => "form-control input-small aqua", "data-wiki-page-id" => "", "data-remote" => $remote))
                    ?>
                    </div>
                    <div class="col-sm-12">
                    <ul class="people-list" id="wiki-update-comment-list">
...
<div class="col-sm-4 col-md-4 col-lg-3 wiki-left-section" >
    ...
    echo $this->element('../Wikies/partials/wiki_comment/get_wiki_page_by_user', $params);
    ?>
...
<style>
#tabContent2 .clear-rating  { display:none !important; }
