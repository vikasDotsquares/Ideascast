            ...
            foreach ($allwikipageusers as $userList) {
                ...
                <li>
                    ...
                    <p class="pagelist">
                        ...
                        <?php $pageCount = $this->Wiki->userTotalWikiPageHistory($project_id,$user_id,$wiki_id); ?>
                        <?php
                        if(isset($pageCount) && !empty($pageCount) && $pageCount >= 1){
                        ?>
                        <a title="Show Wiki Pages" data-remote="<?php echo SITEURL; ?>wikies/get_wiki_page_history_by_user/<?php echo $project_id . '/' . $user_id . '/' . $wiki_id; ?>" data-user-id="<?php echo $user_id; ?>" class="collapsed tipText btn btn-xs btn-default page-list-by-user"  >
                            <i class="fab fa-wikipedia-w" style="cursor: pointer;"></i> &nbsp;<?php echo $pageCount; ?>
                        </a>
                        <?php }else { ?>
                        <a title="Show Wiki Pages" data-user-id="<?php echo $user_id; ?>" class="collapsed tipText btn btn-xs btn-default"  >
                            <i class="fab fa-wikipedia-w" style="cursor: pointer;" ></i> &nbsp;<?php echo $pageCount; ?>
                        </a>
                        <?php } ?>
                    </p>
