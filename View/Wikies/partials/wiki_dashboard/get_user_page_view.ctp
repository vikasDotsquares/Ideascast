	...
	<div class="modal-header"> 
		<h3 id="modalTitle" class="modal-title" >Viewed this page</h3> 	
	</div>
	<div class="modal-body clearfix people">
                <?php
                $allwikipageviewusers = $this->Wiki->getWikiPageViewUsers($project_id, $user_id,$wiki_id,$wiki_page_id);
                ?>
		<?php if(isset($allwikipageviewusers) && !empty($allwikipageviewusers)) { ?>
			<?php foreach( $allwikipageviewusers as $key => $val ) { ?>
				...
				<?php 
                                ...
                                $userDetail =  $this->ViewModel->get_user_data($val['WikiPageView']['user_id']);
                                $user =  $this->ViewModel->get_user($val['WikiPageView']['user_id']);
